...
> decision 5, `F3.37`), and `bms.dashboard_sections`, the screen a template
...
> onboarding may extend it but never edit it (Amendment 1), and `0059` revokes
...
> key**, never an asset id: `bms.asset_group_members.role` into the new global
> `bms.asset_roles` (`F3.37`, migration `0051`), on the membership and not on the
> asset, because the same pump is the raw-water pump in one group and a monitored
> load in another. Six defaults ship as a **stock catalog in the repository**,
...
> hole ADR 0043 Amendment 5 closed. **Instantiation never succeeds silently**
...
| Master data  | Organization → Location → RTU → Asset + `/admin/*` CRUD with `admin`/`organization_admin`/`location_admin` roles (ADR 0008–0010). **The point-key catalog is no longer in that chain** — ADR 0051 decision 2 / migration `0057` made it one fleet-wide list with no `organization_id`, so it is platform vocabulary (§4.4) rather than a child of Organization. Reads stay open to the roles above; **every write is global `admin` only**, except the onboarding commit path, which may *create* a code at `organization_admin` (ADR 0051 Amendment 1) and never edit one — a boundary migration `0059` now enforces in the grants, not only in the service. **ADR 0018** separates the axes: an asset must have a `location_id` (`NOT NULL`) and need not have an `rtu_id` (nullable); telemetry provenance binds at `asset_points.source_kind` (`measured`/`manual`/`computed`/`unmapped`), not at the asset |
...
**The point-key catalog is carved out of that gate and does not use those
predicates** (**ADR 0051** decision 5, `F3.39`). It is not a sixth gate — it is
the master-data gate answering a different question, because `bms.point_keys`
stopped being tenant data in migration `0057`. A scope predicate asks *which
organizations may this user write*, and the catalog belongs to none of them, so
`PointKeysAdminService` gates all four mutations on **`isGlobalAdmin`** instead.
...
- Template content sections whose consumer does not exist yet. **ADR 0019
  ...
  **It reopened at the schema only, and has no tab.** ADR 0038's count is still
  six, and ADR 0050 Amendment 1 decision 5 scoped `E1.3` to the score surfaces
  rather than to a seventh tab. `health` is therefore now in `maintenance`'s
  class — writable by the API, carried through `mergeTemplateContent` byte for
  byte, edited by nothing in the authoring UI. A seventh tab remains an ADR 0038
  amendment, not a side effect. `optimisation` is the one section left.
...
surfaces, so `health` is writable by the API and has no tab, which is
`maintenance`'s shape rather than `dashboards`'. `optimisation` alone is
