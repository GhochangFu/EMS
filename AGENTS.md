...
> *Control Room 2D* group until `F3.66` deleted it, and still wraps the seven
> `/cr-*` routes in `ControlRoomRoute` unless the caller can read a `CR-*`
> asset — until `F3.70` makes them redirects and removes the gate;
> `useControlRoomAccess` now has that guard as its only caller. `F4.156` also clears
...
> `builtin` → the seven `/cr-*` links of `lib/smoc-pages.ts`, filtered by
> `canAccessControlRoomPath`, until `F3.70`'s tabs; `dashboard` → a link card
> to `/dashboards/<slug>` until `F3.69` (a null slug falls back to
...
> refuses a tenant-set rank (Amendment 2, see §4.4). Next: `F3.69` and
> `F3.70` are unblocked; `F4.157` needs its own ADR; `F4.160` is open.
> General
...
│   │                            seven /cr-* links), lib/site-view-notice.ts;
...
│   │                            lib/generated-site-view.ts. The seven /cr-*
│   │                            pages stay in src/pages/ until F3.70.
...
- Functional components only. **One *exported* component per file** — a file's
  ...
  and `control-room-overview-page.tsx` holds 17. A rule the codebase breaks
