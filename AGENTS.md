...
> `value_tile` and one generic `chart`, declared as a `z.enum` and a `CHECK`
...
- **No new `bms.*` table ships without `organization_id` and a
  ...
  and the like) or the stated `telemetry.*` exception (decision 9). A junction
...
- Template content sections whose consumer does not exist yet. **ADR 0019
  ...
  contracted only as far as something on `main` can consume it. **Two** things
  ...
  **`dashboards` left this list on `F3.1a` (ADR 0047)** — it was *"ordered point
  ...
  read/write API), `F3.1c` (the four renderers), `F3.1d` (the builder surface)
  and `F3.1e` (the template *Dashboards* tab) are approved scope under ADR 0047
  and **unbuilt**. Until `F3.1e`, `dashboards` still gets **no tab** in the
  template authoring screen, and the five-tab source scan still holds — that
  count is changed by amending ADR 0038, never by editing the scan
...
registry, held there by a source scan rather than by convention. **Two of those
three statements still stand; the `dashboards` half is now split.** ADR 0047
(`F3.1a`) took `dashboards` out of §6's closed list — it carries widgets — but
it is **still out of the tab registry**, and the source scan still pins five
tabs. `F3.1e` is the row that changes that, by amending ADR 0038. `health` and
`optimisation` are unchanged in both halves.
