...
> (**ADR 0047**, `F3.1a`). **All five children are built and the `F3.1` umbrella
...
> *not needed*.
> **The asset health score is in** (**ADR 0050** and its Amendment 1, `E1.3`,
> 2026-08-30) — a tag's share of samples inside every published threshold rule,
> materialized by a scheduled job into four counter relations, one per ADR 0023
> level, and read as an asset score, a plant donut and an enterprise donut.
...
> answer and not a fault — and on the shipped seed data *every* tag is unruled,
> which `F4.69` exists to fix. And **`health` is the fourth of ADR 0019's five
> content tiers to reopen**; only `optimisation` is still closed.
> General
