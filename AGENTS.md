...
> and evaluation time (**ADR 0055**, `F2.9`, **shipped 2026-09-05** in two PRs;
> `F2.8` **shipped on it on 2026-09-06**: the stock incomer carries `site_kw`,
> `it_kw` and `pue` as `v2` points, the API reads Σ/Σ over the incomers in
> scope or `null`, and the fitted PUE curve is gone from all three copies), the
> **point metadata and the mapping sheet** (**ADR 0056**, `F2.7`, **shipped
> 2026-09-07** in two PRs: five nullable scale, offset, engineering-range and
> quality-policy columns on template points and asset points, the asset
> overriding the template per column and the ingest host applying them in a
> fixed order with an `outOfRange` counter; an Excel mapping sheet per
> location with preview-then-commit that writes the valid rows and skips the
> rest; an all-or-nothing bulk editor of up to 500 points; and the instantiate
> dialog collecting pattern variables, which closed `F4.56`), the **alarm
> lifecycle** (**ADR 0057**, `F3.10`, **shipped 2026-09-07** in two PRs: an
> alarm is active while `cleared_at IS NULL` and acknowledgement is an
> annotation; one sweep on the shared loop auto-clears after a per-rule hold
> and escalates through per-organization, per-severity profiles in four
> tenant tables; the cleared and escalation messages ride the notification
> dedupe key; **Amendment 2**, `F3.48`, **2026-09-08** — a step the hourly
> ceiling refuses now writes no delivery row, so its key survives and the next
> tick retries it, and a rate-limited row no longer blocks an event key),
> the **alarm philosophy surfaces** (**ADR 0059**, `E2.2`,
> **shipped 2026-09-08** in two PRs on one ADR plus a post-merge fix: the
> class philosophy a template alarm carries now reaches the operator on the
> alarm details panel, resolved through `E2.4`'s provenance, and a browsable
> knowledge base at `/alarm-kb` lists it per published asset class — open to
> `viewer`, which is the point, since the same content sits behind a
> master-data gate on the authoring screen) — and
...
> nothing* — except the escalation step an hourly ceiling refuses, which since
> `F3.48` writes none so that a later tick can retry it — a refusal recorded **once per channel, organization and dedupe key**
> rather than once per press since `F3.46` (ADR 0041 Amendment 2, migration
> `0065`'s partial index), an egress guard in front of every webhook, and the
> channel admin screens shipping inside the row rather than after it (**ADR 0041**, `F3.8`;
> **`F3.7` shipped on it on 2026-09-06** — both raise paths dispatch a `notify`
> rule, the streaming engine only on a transition, the sweep cross-organization
> as the sweep itself is, and a per-rule channel picker on the rules card,
> ADR 0041 Amendment 1),
...
> `pgsty/minio:RELEASE.2026-08-04T00-00-00Z` since MinIO's own `quay.io`
> repositories started answering 401 — **ADR 0066 Amendment 5**,
> 2026-09-24 — Docker Hub still does not host `minio/minio`), the tenant table
...
> And **per-asset default dashboards** — the gate ADR 0019 and ADR 0047
> decision 6 both reserved for `F3.2`: `bms.dashboards` gains an `asset_id`
> scope (cascade) and an `asset_template_id` version stamp (no cascade), a
> three-way count-form scope check, two stamp checks and a five-leg tenant
> policy (migration `0073`); a template's `content.dashboards` views
> materialise into dashboard, widget and point rows for each asset, inside
> the asset-instantiation transaction after the rule seed and through a
> chunked, resumable backfill route on the published version; all 27 stock
> classes carry an `overview` view; the dashboards list shows `Asset · <code>`
> and the template page carries the action with its report (**ADR 0067**,
> `F3.2`, 2026-09-17, PR #463 — seven owner rulings, four reviews applied
> before merge). Not a §6 promotion: `F3.2` was a deferred backlog row, and
...
> Active with client-side filters, and a side panel that shows the asset's
> health and its `F3.2` default dashboards as viewer links. `GET /assets`
> gains six static columns (the `rtus` join carries an organization
> predicate; `telemetrySource` is reported, never derived) and
> `GET /dashboards` gains `assetId`, narrowing within the caller's read scope.
> The route is not `/assets` because Vite emits the bundle there and nginx
> answered 301 → 403 — a defect only the browser layer could see (**ADR 0068**,
> `F3.31`, 2026-09-17, PR #466 — eight rulings, four reviews applied before
> merge). Not a §6 promotion; the sidebar position moves with the §5
...
> And **the chart legend names the asset**: `dashboardWidgetPointDtoSchema`
> carries `assetCode` (`varchar(64)`, required), read off the `bms.assets` join
> `resolveBoundPoints` already made for its organization predicate — no second
> fetch — and a chart series is always `<assetCode> · <pointKey>` through one
> named `seriesNameFor`, so ECharts, which keys the legend by name, shows five
> breakers' `kw` as five entries (**ADR 0069**, `F3.43`, 2026-09-18, PR #486 —
> four rulings, three reviews and a post-merge sweep applied). The
> section-template instantiate read-back, a second producer of point rows the
> ADR's own sweep missed, now goes through `resolveBoundPoints` too. Not a §6
> promotion; the inspector labels and the picker row are unchanged.
> And **the calc engine reads named parameters** (**ADR 0070**, `E4.1a` ⭐,
> 2026-09-19, PRs #492 and #493 — six gate questions, two ruled against the
> recommendation; ten plan rulings; eight reviews and a post-merge sweep
> applied): `bms-calc-v3`, a strict superset of `v2`, adds the `$key`
> parameter reference, resolved by the scheduled sweep from a new tenant table
> `bms.calc_parameters` (migration `0074`; nearest scope wins — asset, then
> location, then organization — effective-dated, **no default value anywhere**:
> an unset key is a counted `parameter_unset` that writes nothing) against a
> global vocabulary `bms.calc_parameter_keys` seeded with twelve stock keys
> and **no values**, because B14 is still unanswered. `/admin/calc-parameters`
> is the write path, authorised by the row's scope. **And time windows**
> (`E4.1b`, 2026-09-19, PRs #497 and #498 — fourteen plan rulings, two
> against the recommendation; seven reviews and a post-merge sweep applied;
> ADR 0070 Amendment 1): `sum`/`avg`/`min`/`max`/`delta` over one point
> reference and a rolling (`24h`, up to `366d`) or calendar (`today`,
> `this_week`, `this_month`, `this_year`) window, plus `hours(window)`,
> read from the continuous aggregates by a pure segment planner (the
> coarsest aligned level behind its watermark, finer levels for the head and
> the tail) — `sum` is the time integral (`avg × hours`), `delta` is two
> index probes on raw rows; a calendar window is evaluated in the owning
> asset's location's zone, `bms.locations.timezone` (migration `0075`,
> nullable, **no default**, validated against `pg_timezone_names`, seeded
> `Africa/Johannesburg` for the Eskom demo and `Asia/Kolkata` for the PHE
...
> complete. **`F3.5b` landed 2026-09-22 (PR #513, ADR 0071 Amendment 2)**:
> `bms.report_schedules` (migration `0078`, FORCE RLS; `formats`, `cadence`,
> `run_at_local`, an IANA `timezone`, `location_ids`, a same-organization
> `email` channel, `next_run_at`), the `reports-dispatch` (fleet, repeatable,
> `FOR UPDATE SKIP LOCKED`, enqueue before advance) and `reports-render`
> (tenant, idempotent on `(schedule_id, period_end, format)`) queues on the
> worker, `NotificationMessage.attachments` through `EmailTransport` only, the
> outcome on `report_files.delivery_status`, five `/api/v1/reports/schedules`
> routes, and a Schedules section on the Reports page. The worker now imports
> `StorageModule` and the compose `worker` carries the `OBJECT_STORAGE_*`
> lines. Deferred by name: catch-up of missed periods, a run-now route, the
> orphan sweep (`F4.147`).
> And **the sustainability section is a surface, not a report** (**ADR 0072**,
> `E4.2`, 2026-09-23, PRs #519/#520/#521/#522 — seven gate questions and eight
> plan questions, all ruled as recommended; eight reviews and two post-merge
> sweeps): `sustainability.total` and `sustainability.by_location` are the
> first `METRIC_CATALOG` entries carrying fields on their write schema
> (`{ pointKey, aggregate }`), so `bms.dashboard_widget_sources.params` —
> stored since ADR 0047 and read by nothing — has a reader. Each aggregates
> the latest sample per carrying **active** asset in scope inside a freshness
> bound (3 × a scheduled derived point's interval, a flat 15 minutes for a
> measured one) and answers `coverage { fresh, carrying }`, which the tile
> renders as `n of m assets` and the benchmark table as `n/m`; `currency`
> comes from `MONEY_POINT_KEY_CODES`, an explicit list, because 247 catalog
> codes carry an empty unit and "money" cannot be inferred from one. Migration
> `0079` widened the `dashboard_widget_sources.catalog_key` CHECK that `0054`
> had frozen. Fourteen new point keys and twelve `*_this_month` / `*_this_year`
> stock rows on eight classes; `sustainability-overview` at `stockVersion: 2`
> with eighteen widgets; `GET /dashboards?section=` (a LEFT join, so a
> hand-built dashboard survives the unfiltered list, with the organization
> predicate inside the JOIN because the fleet branch is `BYPASSRLS`); and the
> *Sustainability* sidebar entry with its `/sustainability` entry route.
> **ADR 0072 decision 1 amends ADR 0049**: a section template could instantiate
> only into an asset group (`asset_groups.location_id NOT NULL`), so a
> role-free template now instantiates **organization-wide** with both scope
> columns `NULL`, and a template that binds a role still refuses with a 400.
...
> `sum` rows say *estimated over the whole period* (thirty-four since `E4.3`
> added ten `outlet_kl_*` window rows), and `E4.4` built the guard
> (`window_sparse`, above).
...
> And **the water tiles sum intake only** (**ADR 0073**, `E4.3`, gated
> 2026-09-23 in #530, closed 2026-09-24 in #538; PRs #531/#534/#536 with
> sweeps #533/#535/#537 — seven gate questions, Q1 and Q6 ruled against the
> recommendation; twelve plan questions, all as recommended; three rulings at
> the PR 2 review and sweep, R1–R3 at PR 3's): v3 double-counted water, one
> inlet per stage of a series plant. Migration `0080` adds the seventh
> vocabulary, the global `bms.water_balance_roles` (`intake`, `discharge`,
> `reuse`, `internal`), and `bms.assets.water_balance_role` (FK, no default,
> `NULL` = out of the balance); an optional `balanceRole` on
> `sustainability.total` / `by_location` narrows the carrying set and its
> coverage. `sustainability-overview` v4 has 19 widgets: `balanceRole:
> "intake"` on its four water bindings and a *Water balance by site* table on
> the new `water.balance` dataset at `this_month` (migration `0081` widened the
> `catalog_key` CHECK to eight keys; `0054` and `0079` stay frozen). Its
> columns are Intake, Reuse, Discharge, Consumed or lost = Intake − Discharge
> (reuse is not added) and Coverage. `consumed` is `null` when any intake or
> discharge asset is stale or does not carry the period's key, and `intake − 0`
> only where no asset is `discharge`; coverage is `fresh/carrying` over intake,
> reuse and discharge; a site is a row only when it owns one of those three.
> `outlet_kl_*` sits on five water classes at v5; the softener stays v4 (its
> totalizer resets). The demo plant is five `WTR-*` assets at CSMOC Gauteng on
> `DEMO-WATER-<CLASS>` mirror templates, held byte-equal to the stock modules
> by a drift test, with `water: "WATER"` in `DOMAIN_RTU_SUFFIX` and four exact
> `verifyHierarchySeed` counts scoped to the demo codes (R1); `stepWater`
> emits for `WTR-` codes only (R2), `assignEskomAssetRtus` wires only `WTR-`
> water assets (R3), and a local sim run needs `SIM_ASSET_COUNT=all`.
> Residuals: `F4.151`, `F4.152`, `F4.153`; Water Recycle % is drafted, not
> shipped, while B14 is open. Not a §6 promotion: no §6 line named this surface.
> And **domain dashboard parity with the client reference layout** (**ADR
> 0074**, `F3.28`, gated 2026-09-24 under #541; three serial slices, #542,
> #543, #545 — ten gate questions ruled as recommended, Q1 answered outside
> its options): the §5 *Reference layout language* gate is resolved — **the
...
> `assetIds`, both intersected with the caller's readable assets; a new
> active-alarm-count-by-severity read; and an Active Alarms rail on
> `/cr-overview` replacing `ActiveRulesPanel`. Slice 2 (#543): `GET
> /api/v1/dashboard/kpis` gains a nullable `prior` for `totalKw`, `alarmsOpen`
> and `pueEstimate` at `asOf − 24 h` (`sitesOnline` gets none), and a new
> bounded point-values-at-instant read, both reusing `F3.35`'s `formatDelta` /
> `KpiTile` hint slot on `/` and `/cr-overview`; `totalKw` and its prior stay
> unbounded by design, but **Errata 1** keeps `pueEstimate`'s live 900 s bound
> (`PUE_LATEST_MAX_AGE_SECONDS`) and applies it to the prior too, as
> `(at − 900 s, at]` — the owner's OQ2 ruling. Tiles that count a live state
> (*Rule Warnings*, *SLD Status*) get no delta. Slice 3 (#545): `GET
> /api/v1/assets/role-summary` (declared before `:assetId/points`) reads an
> ADR 0049 role per `code`/`label`/`count`/`worstSeverity`/`worstCount`/
> `offlineCount`, ordered by `sort_order`; a membership counts only when the
> caller can read its group, an inactive asset is excluded, and an asset
> holding two roles is counted under each. Offline means no sample of any
> point in `LIVE_TELEMETRY_MAX_AGE_SECONDS` (25 s,
> `apps/api/src/telemetry/telemetry-freshness.ts`), pinned to the web gate's
> `FRESH_MS` by `tests/f3.28-offline-bound-single-source.test.ts` — the two
> gates can still disagree on one asset, since the strip judges any point and
> the web gate judges each reading. The window is a **plan-time SQL literal**,
> not a bound parameter: measured at 520–1900 ms per call bound, 42–100 ms
> literal, because a bound interval stopped TimescaleDB excluding chunks.
> Slice 3 also shipped the state legend (`alarmSeverities` from `GET
> /api/v1/vocabularies` plus *Normal* and *Offline*, no *Standby*), the
> Diagram/List toggle reusing `BreakerTable` (extracted from `/cr-sld`), four
> Key Parameters gauges (the `F3.1c` renderers, radial only — `/cr-overview`
> has no tank asset), and a static footer ribbon. Not a §6 promotion: `F3.28`
> was a deferred backlog row (ADR 0047 decision 6, ADR 0067). New rows:
> `F3.65` (the user theme switch) and `F4.154` (the "vs yesterday" label on a
> non-24-h `compareToPrevious` tile, found in flight and not fixed here).
> And **System Status and Data Quality indicators** (**ADR 0075**, `F3.30`,
> gated 2026-09-25 under #548 — nine gate questions ruled as recommended;
> built in #552, squash `65cb22ed`; closed in #553): `GET
> /api/v1/system/status` (`apps/api/src/system-status/`, JWT-guarded, open to
> every role, scoped through `readableAssetIds` + `withReadScope`) answers
> three component states — `queue` and `storage` from the existing health
> providers, and `field_data`, inferred as "at least one in-scope asset on a
> `source_type = 'mqtt'` RTU reported" (`not_monitored` when there is none)
> — one verdict (`operational` unless a component is `degraded`;
> `not_configured` and `not_monitored` never degrade it), and `dataQuality`
> = the reporting share of in-scope assets **with an RTU**. The body carries
> **state only** — no queue depths, bucket names, heartbeat times or error
> text. There is **no `database` component**: every `/api/v1` read resolves
> the caller's scope from the database first, so a failed request *is* the
> database check. The contract is `systemStatusResponseSchema` (ADR 0030).
> **Two windows, on purpose.** The four `kw`-only freshness tests —
> `locationKpis`' and `locationDashboard`'s RTU rows' `fresh_asset_count`,
> `kpis.sites_online` (20 s → 25 s) and the map's comm status — moved to the
> ADR 0074 any-point rule through one shared `LIVE_ASSETS_CTE_SQL` built from
> `LIVE_TELEMETRY_MAX_AGE_SECONDS` (25 s); `total_kw` did not change. The
> status read alone uses **Amendment 1**'s `REPORTING_WINDOW_SECONDS` (150 s,
> `REPORTING_ASSETS_CTE_SQL`), because the real MQTT devices report every
> 60 s and under 25 s a healthy feed read 4–12 of 20 fresh: the cards ask "is
> this reading live?", the status bar asks "is data arriving?", and the two
> may differ on the same fleet. `tests/f3.28-offline-bound-single-source.test.ts`
> pins every freshness bound to its constant and forbids the status service
> from reading the live one. The web side is a `SystemStatusIndicator` beside
> `StatusBarClock` in `app-shell.tsx`'s footer — the bottom status bar, not
> the sidebar, so it stays clear of the open §5 *Domain-first navigation IA*
> decision — polled every 30 s with a 10 s request timeout and **exactly one
> retry** (`retry: 1`, the app's only per-query override of
...
> `F4.159`: the `/` Total kW for a global user still summed `kw` from asset ids
> with no `bms.assets` row. **`F4.159` fixed the whole class on 2026-09-26**
> (#561; the owner widened it from two reads to ten): telemetry has no foreign
> key to `bms.assets`, so every kW and PUE sum now joins `bms.assets` inner in
...
> `F3.66`–`F3.70`, `F4.157`). The interim gate **`F4.156`** (#550) hid the
> *Control Room 2D* group until `F3.66` deleted it, and wrapped the seven
> `/cr-*` routes in `ControlRoomRoute` unless the caller could read a `CR-*`
> asset until `F3.70` (#574) made them redirects and deleted the gate
> (`ControlRoomRoute`, `useControlRoomAccess`, `canAccessControlRoomPath`,
> `hasAnyControlRoomAsset`). `F4.156` also clears
> the query cache on any session change (`bindQueryCacheToSession` in
...
> **`F3.66`** (#565, squash `884bc0ce`; closed in #567; owner rulings OQ1–OQ4)
> is the shell: one **Control Room** sidebar entry in *Operations*, after
> *Alarm Centre*, shown to every scope but `none` (not while the scope is
> still loading), replaces the seven-item *Control Room 2D* group. Three
> levels, `/control-room` (the organization list),
> `/control-room/org/:organizationId` (the site cards and an
> `ActiveAlarmsRail`) and `/control-room/site/:locationId`, each skipped when
> it holds one item (`lib/control-room-levels.ts`, decided only from a
> resolved read) and guarded by `ControlRoomScopeRoute`, which keeps a `none`
> scope out — the API is the access control on every level. The site page
> renders the `F3.67` resolve read by `kind`: `generated` → `F3.68`'s view;
> `builtin` → at first the seven `/cr-*` links of `lib/smoc-pages.ts`,
> filtered by `canAccessControlRoomPath` (since removed), until `F3.70`'s tabs
> (below); `dashboard` → at first a link card to `/dashboards/<slug>`, until
> `F3.69` rendered the dashboard inline (below; a null slug falls back to
> generated); a rejected read → the "not available in your access scope"
> card, never a body. `GET /api/v1/alarms` and `/alarms/summary` take an
> optional `organizationId` so the rail sends one uuid, not every asset id
> against the 200-id cap; it is ANDed onto
> `intersectReadable(readableAssetIds(user), …)` and never replaces it, so an
> organization outside the scope answers empty, not 403. **`F3.68`** (#569,
> squash `e669ba54`, merged; plan owner rulings OQ1–OQ4) is the generated
> view: `bms.point_keys.headline_rank smallint NULL` (migration `0083`, CHECK
> `> 0`; lower first, ties by `point_key`), filled only where `NULL` by
> `seedPointKeyHeadlineRanks` after the last catalog writer (on/off `10`,
> `kw` `20`, `kwh_total` `30`, `pf` `40`, `frequency_hz` `50`) and edited on
> `/admin/point-keys`; `GET /api/v1/control-room/sites/:locationId/generated`
> (`GeneratedSiteViewService`, `FLEET_POOL`, the `F3.67` readable rule and
> its 404) answers one panel per domain, one card per readable asset with
> its registered points in headline order and each point's latest sample
> within a **literal 7-day window** (`GENERATED_LATEST_WINDOW_SQL`; 69.7 s
> unbounded on `RSMOC-WC`; bounded, 31–41 ms to plan and ~13 ms to run —
> Amendment 2's owner ruling);
> `telemetryFreshnessAt` in `telemetry-freshness.ts` is the one JS freshness
> judgement it shares with `DashboardService`. `GeneratedSiteView` shows the
> location KPIs, then four points per card and "All points" inline, live
> over one socket; **a stale value stays, dimmed** (Amendment 1 — this view
> only, ADR 0027 decision 3 holds everywhere else), and migration `0084`
> refuses a tenant-set rank (Amendment 2, see §4.4).
> **`F3.69`** (#572, squash `f433ad25`; closed in #573; owner rulings OQ1,
> OQ2) is the dashboard site view (decision 8): `DashboardLiveCanvas`
> (`components/dashboards/`) is the dashboard viewer's live canvas as one
> component, and `SiteDashboardView` (`components/control-room/`) reuses it
> to render the site's dashboard inline under the site page's breadcrumb,
> header and notice. It reads the dashboard under the viewer's query key with
> the site's organization id and decides from the data: a failed background
> refetch keeps the canvas, and only a read that never answered shows an
> inline alert whose *Try again* refetches the dashboard and invalidates the
> `["control-room", "site-view"]` resolve prefix (OQ2). The fail-safe stays
> in the API; the view never falls back to the generated view itself. One
> *Open in Dashboards* link, no Edit link (OQ1). No route, contract or
> migration. New row: `F4.161` (P3 — `readableOrganizationIds` and
> `scopeForUser` stop their grant walks on different conditions, so one
> mixed-grant reader opens a site but gets a 404 on its dashboard; it fails
> closed). **`F3.70`** (#574, squash `852cc1b1`; closed #575; plan owner rulings
> OQ1–OQ6) is the built-in SMOC view (decision 9): the seven SMOC pages are
> tabs of one route, `/control-room/site/:locationId/:tab?`
> (`overview`, `sld`, `ups`, `battery`, `hvac`, `env`, `it`; no segment =
> `overview`), listed in `SMOC_TABS` in `lib/smoc-pages.ts`. They show only
> for a `builtin` view on the SMOC site, `RSMOC-WC` **in `ESKOM`**
> (`isSmocSite`) — a location code is unique only per organization
> (migration `0016`), so the code alone does not name the site; a `builtin`
> view on any other site renders the generated view. `SmocSiteView`
> (`components/control-room/smoc-site-view.tsx`) draws the strip over
> `allowedSmocTabs(scope)` — the per-area asset-group rule,
> `canAccessControlRoomArea`, keyed per tab — and the chosen tab's content
> under **one** `SchematicTelemetryProvider` for all seven; a tab outside the
> rule keeps the strip and shows "Outside your asset-group scope", with
> neither the content nor the provider mounted. The seven contents
> (`ControlRoom<X>Content`) moved to `components/control-room/smoc/` without
> their `AppShell` wrappers. Once both reads have data, a tab segment on a
> site that does not show the tabs, or an unknown tab, redirects to the bare
> site path. The seven `/cr-*` routes stay as redirects inside
> `ControlRoomScopeRoute`: `SmocLegacyRedirect`
> (`components/smoc-legacy-redirect.tsx`) finds the SMOC site in the caller's
> readable KPI list — the location KPI row gains `code` for it — and sends
> the caller to its tab, or to `/control-room` when the list lacks it or the
> read fails. Access is "can read the site"; the `F4.156` gate is deleted,
...
| Layer        | Technology |
|--------------|------------|
| Frontend     | React 18, TypeScript 5, Vite, Tailwind CSS, TanStack Query, Zustand, React Router, Leaflet, ECharts, and — since `F2.5` (ADR 0038 Amendment 2) — **CodeMirror 6** on the two authored-formula surfaces only. Five declared packages (`codemirror`, `@codemirror/{state,view,autocomplete,lint}`), composed from `minimalSetup` and never `basicSetup`, reached solely through `components/asset-templates/formula-editor-lazy.tsx` so the library ships in its own chunk. Measured: the entry chunk contains no CodeMirror at all, and `@codemirror/search` tree-shakes out of both chunks. `tests/adr-0038-formula-editor.test.ts` holds all of that statically — it is the only module allowed to import `codemirror` or `@codemirror/*`, in any import form. **Whether an asset is live is decided in exactly one place in this client since `F4.37` (2026-08-14): `apps/web/src/lib/schematic-telemetry.ts`.** (Since `F3.28`, 2026-09-24, the API judges liveness too, independently, for `GET /assets/role-summary`'s per-role offline count — pinned to the same `FRESH_MS` value by `tests/f3.28-offline-bound-single-source.test.ts`, but not sharing this code path; see the ADR 0074 status-line paragraph. Since `F3.30`, 2026-09-25, the same 25 s any-point rule also decides the location cards' fresh count, `/` Sites online and the map's comm status, through one `LIVE_ASSETS_CTE_SQL`; only the status bar's `GET /api/v1/system/status` uses a separate 150 s reporting window — ADR 0075 Amendment 1, see its status-line paragraph.) `FRESH_MS`, the arrival clamp and `isStale` live there, extracted from the context component so they can be tested at all — the context imports React, TanStack Query and socket.io-client, and `vitest.config.ts` only counts `apps/web/src/lib/**` toward coverage, so anything above it is untestable *and* invisible to the gate. Put new pure logic there, not in the component. **Freshness is computed at render, so it needs something to force one**: the provider's `staleTick` is the only periodic re-render in the app (**since `F3.68` there is a second**, outside the provider: `hooks/use-site-live-readings.ts` runs its own `STALE_TICK_MS` interval for the generated Control Room site view; it only forces the render, reads `Date.now()` at render, and judges through the same `isStale` and `readingTimestampMs`), and a `refetchInterval` is not a substitute — TanStack v5 tracks accessed properties and structurally shares results, so an unchanged response notifies nobody. **Since `F4.38` (2026-08-15, ADR 0027) the gate reaches everything on screen**, not just the SVG schematics: all seven control-room pages derive their tiles through `isStale`, a stale tile renders `—` rather than its last numbers (**one named exception**: ADR 0076 Amendment 1, `F3.68` — the generated Control Room site view keeps a stale point's value, dimmed, per row through the same `isStale`; do not "fix" it to `—`, and do not copy it to another page), `offline` outranks `critical` in every page banner, and aggregates (`ctx.totalKw`, the KPI averages) exclude stale slices and show the count they excluded. Two rules follow for anyone adding to these pages. **Status renderers are `if`/ternary chains whose default is the healthy branch, so a new status member compiles silently and draws as `normal`** — test `offline` first in every chain; the compiler will not find them for you. And **read the clock at render**, taking the re-render from the provider's `staleTick`: a page that starts its own interval or caches the status re-freezes the tiles. `tests/repo-invariants.test.ts` holds both, plus the live-critical count that stops a dead sensor masking a live alarm. **`F4.39` (2026-08-15, ADR 0028) closed the assumption underneath all of this — that the thing on screen is a reading at all.** Every value on a control-room page is now one of: *measured* / *derived* (gated by ADR 0027), or *nameplate* / *configuration* / *simulated*, which render through `StaticValue` / `StaticTspan` (`components/static-value.tsx`) and are visibly marked `NP` / `SET` / `SIM`. The rule that decides which: **a value may be labelled a measurement of X only if it comes from telemetry that measures X** — `kVA` from `kW` and `pf` is fine, "Voltage Y" from Voltage R is not, and 32 cell voltages from one string voltage is not. Markers qualify *values*, not headings, hints or `x / y` denominators, whose form already says they are not readings. Three more traps this drew out: **each value takes the clock of the asset it came from** — `freshValue(own ?? fallback, ownStale)` reads naturally and is wrong, because the `??` resolves before the gate, so use `ownElse`; **absent is not zero** — `(fanSpeedPct ?? 0) > 20` renders a unit that publishes no fan speed as `IDLE`, which for a standby unit is its normal reading, so use `isHvacRunning`, which returns `null`; and a box holding a gated value must be able to **render offline**, or an em-dash inside a confident green outline is the only signal. Checks live in `tests/repo-invariants-provenance.test.ts`. **Since `F4.23` (2026-08-15, ADR 0030) every response this client reads is checked against a schema before any of the above sees it** — see the *API contracts* row and §4.8; a `fetch` in `src/api/` that does not go through `checkResponse` is the gap that row exists to close |
...
| Point metadata & mapping sheet | Since `F2.7` (**ADR 0056**, 2026-09-07) every measured point may carry **five nullable metadata columns** — `scale_multiplier`, `scale_offset`, `eng_min`, `eng_max`, `quality_policy` — on `bms.template_points` as the class default and on `bms.asset_points` as the per-asset override, resolved `coalesce(asset, template)` per column (ADR 0039's calc-config shape); a resolved `NULL` is today's behaviour (×1, +0, no band, `discard_bad`). **Two layers hold the rules, and they are different rules:** the database holds the *within-row* ones as CHECKs — `eng_min < eng_max`, `scale_multiplier <> 0`, the policy enum (`0063`), and finite values in `0031`'s range form (`0064`, because Postgres sorts `NaN` above every float so `<` and `<>` admit it) — while `apps/api`'s Zod layer holds the *merged pair* (`validateMergedPointMetadata`, naming the inherited bound), which no row CHECK can see. **The ingest host applies them in one fixed order per target — quality policy, scale, finite, range — and drops with a counter, never a stored quality mark**: `telemetry.point_values` is untouched and ADR 0050's "in safe range" stays threshold-based; the engineering range is an instrument band, not an operating limit. Scaling applies to adapter samples only — `F1.8`/`F1.9` values are engineering values already. `BINDING_QUERY` coalesces the five over a LEFT JOIN to `template_points` (measured: 1.56 → 1.70 ms). **The mapping sheet is one workbook per location** (`GET /admin/asset-points/mapping-sheet.xlsx?locationId=`): existing non-computed rows plus pre-fill suggestions from `source_data_key_pattern` with `{asset_code}` substituted and other tokens literal, `active` blank meaning "not taken"; `POST …/mapping-sheet/preview` returns creates, field-level updates, unchanged, untouched suggestions and per-cell errors from **one closed list of 24 codes** (`MAPPING_SHEET_ERROR_CODES`, with the web's labels `Record<code, string>` so a new code fails `pnpm build`); `POST …/mapping-sheet/commit` writes the valid rows in one transaction and skips the rest (the owner's ruling, against all-or-nothing), re-keys in three phases so a key swap cannot trip `asset_points_asset_source_key_idx`, and audits each written row through `MasterDataAuditService.writeMany`. Export → import is an **identity**, gated by `assertExportedRowsPlanAsIdentity` and by the integration suite on a fixture location — which is why an empty stored unit reads as `null` (`storedText`) and a retired RTU's code is accepted only where the row already points at it. **The in-app bulk editor is all-or-nothing** (`POST /admin/asset-points/bulk-update`, ≤ 500 ids) — it has no preview to make skipping safe; that asymmetry is deliberate and flippable by amendment. **Every spreadsheet upload runs two guards before it costs anything** (`apps/api/src/admin/spreadsheet-guard.ts`, shared with `F1.9`'s importer): `zipInflationProblem` reads the zip central directory and refuses a declared inflation over 64 MiB or zip64 sizes before `XLSX.read` inflates a byte (a 1.3 MB file reached 2.5 GB RSS without it), and `quoteCell` bounds every echo of sheet text in a response message to 64 characters plus the omitted length (6,000 duplicate rows with 32,767-character cells produced 375 MiB of messages and ~32 s of blocked event loop without it) — apply `quoteCell` at each interpolation site, never in a wrapper, because two messages are built before their wrapper runs. **A third guard bounds a sheet's declared *column* span**, added by `F4.101` (2026-09-07): `sheet_to_json` densifies every cell of the *declared* range on both axes and `sheetRows` bounds rows only, so a workbook declaring `<dimension ref="A1:XFD20102"/>` while holding two real cells cost 329,351,168 cells and **killed the process** — `JavaScript heap out of memory`, from a 2,668-byte upload. `MAX_HEADER_COLUMNS` (64, the owner's ruling) bounds the read to a window counted from the used range's own first column, and a recognised header beyond that window refuses the file **by name** rather than being dropped. **This section claimed until `F4.101` that `mapping-sheet-rows.ts` escapes the problem by addressing cells over a fixed header span instead of densifying. It does not.** Densification was never the only amplifier: that parser's *header scan* walked the declared width, and ran before the row cap, so a 2,465-byte upload declaring 321,272,406 columns killed the process there too — it now scans the twelve columns plus one. **The lesson generalises past spreadsheets: any loop bounded by a number the input declares is unbounded work, whether or not it allocates per cell.** **All three upload routes now carry all three guards** — `F4.102` (2026-09-07) brought the onboarding upload level, where a **3,038-byte** workbook had killed the process, and gave its `FileInterceptor` the `limits` it had never had; multer defaults `fileSize` to `Infinity`, so an absent `limits` object is an unbounded upload. `tests/f4.102-file-interceptor-limits.test.ts` now fails any controller whose upload interceptor omits `limits.fileSize`, and it is the gate on *declaring* the limit only: no test in this repo instantiates a Nest module, so *enforcement* is checked at the API layer or nowhere. **A per-cell bound is not a per-count bound.** `quoteCell` caps what one interpolation costs; the number of interpolations is still whatever the input declares, and after `F4.102` a 1.75 MB upload still returns a 13.16 MB message (`F4.105`). **A read bound is not a semantic limit either.** `SHEET_ROWS_BOUND` bounds what `XLSX.read` materialises and nothing more, so until `F4.103` (2026-09-08) a sheet one row under it parsed to 20,090 assets that reached `bms.onboarding_sessions.draft` and then ~40,000 sequential statements inside one `withTenant` transaction; the sibling importer's `MAX_IMPORT_ROWS` is the limit, the bound is not. The four draft arrays are now capped at 100 RTUs / 500 assets / 500 point keys / 5,000 asset points (`packages/shared/src/contracts/onboarding.ts`, imported by `apps/api` so the two write paths cannot drift), worst case 6,700 statements. **Count the producers, not the routes.** `F4.103` enumerated three writers of the onboarding draft from the source and there were four: `handleRuleBasedTurn` is the deterministic branch ADR 0011 ships when no OpenAI key is set — the **default** path, since `.env.example` ships `OPENAI_API_KEY=` empty — and it builds its patch in code without ever calling `safeParse`, so the schema `.max()` never saw it. A guard placed on "the routes that write X" is only as good as that enumeration, and a second producer reaching the same store through a different door is invisible to every test that exercises the first. **A repeated `SELECT` in a per-row loop is unbounded work too**: `assertAssetDomain` is an uncached single-row read and ran once per asset before the transaction opened, which the `F4.103` row's own cost figure omitted entirely. **Bound sheet text at the parse boundary, not at the echo**, where a cell has a column that already constrains it: `topic` is refused over 255 because `bms.rtus.mqtt_topic` is `varchar(255)`, and the RTU `protocol` cell is `safeParse`d rather than cast, because Zod's `invalid_enum_value` embeds the **whole** received value while `too_big` and regex messages do not — a cast there returned 65.8 MB from a 231 KB upload. **Count the interpolation sites from the source, never from a green suite.** `F4.102` enumerated four, review found a fifth, post-merge review found a sixth, and twelve revert-and-measure-red results were honest throughout: the hostile fixture pinned the missed field to `""`, so no assertion reached it. A revert-check proves an assertion depends on the code you reverted; it never proves the enumeration is complete. **A sheet is read by its absolute position, never by its offset within the used range** — `sheet_to_json` indexes both axes from `!ref`, so an index taken from its output must have `range.s.r` / `range.s.c` added back before it addresses a cell; `F1.9`'s importer shifted every timestamp by a row, and by a column, until `F4.100` (2026-09-07). Only a plain decimal literal is a number in a cell (`0x10` is `number_invalid`, not 16). **A `{token}` in a source-key pattern has one grammar**, `packages/shared/src/source-key-pattern.ts`, read by the instantiate service, the migrate service, the sheet's pre-fill and the instantiate dialog (`F4.56`, closed here: one input per variable per asset). Vars are still not persisted (ADR 0039 Q-A); the sheet is the correction path. The single-row Add/Edit form is a whole-object editor (`F2.31`); the template version delta does not yet report a changed default (`F2.24`) |
| Ingest adapters | `IngestAdapter` interface frozen by **ADR 0016**: the host owns *supervision and cadence* (poll loop, overlap guard, backoff, jitter, bounded queue, process lifetime); adapters own the protocol connection and parse, implementing `connect` / `disconnect` / `health`. **The host is now built** (§6 commit 2): `apps/ingest/src/host/` supplies the supervisor, bounded queue, binding plan, normaliser and health endpoint, `src/main.ts` is wiring only, and **the §5 backoff table itself moved to `packages/shared/src/ingest.ts` on 2026-08-14 (`F4.34`)** because the API's telemetry listener became its second consumer — the ADR states those numbers precisely "so five agents do not invent five policies", so a second copy would have defeated the point of writing them down; change them in one place and both the ingest supervisor and the API listener follow, and `src/adapters/mqtt.ts` ports the pilot's MQTT connection onto the interface behind `src/adapter/registry.ts` — a port that **deliberately diverges** from the ADR 0007 pilot's parser in three ways, listed in `docs/ingest-host.md`. That happened because `index.js` was frozen while it served the pilot, so a defect found in the shared parse logic could only be fixed on the host side; §6 commit 4 has since deleted it, and the divergence list is kept as the record of what the host does differently from the behaviour that ran in the field — it is what explains the step change in the pilot's data on 2026-08-06. **Since 2026-09-06 the bounded queue has a disk tier under it (`F1.10`, ADR 0016 Amendment 4):** `host/disk-buffer.ts` spills a batch whose database write fails to a JSON-lines segment under `INGEST_BUFFER_DIR` — a named Compose volume, and a host that cannot write there refuses to start — bounded at 1 h by receipt time *and* 256 MiB host-wide, both enforced after every append and on a once-a-minute sweep; the supervisor then stops attempting the database, probes on the same §5 backoff, and replays oldest-first behind live writes, idempotently, when a write succeeds. The health line carries `writePath=`, `buffered=`, `bufferDropped=` and `replayed=` per endpoint and reports `degraded` while any of them says loss; `prom-client` stays deferred to `F3.16`. The buffer survives a *database* outage — when the broker is down nothing reaches the host, and that window is `E7.2`. A host restarted while the database is still down exits before any supervisor (the binding plan needs the database), so the segments wait on the volume for the next start. **MQTT is the only implementation, and it is not new scope** — ADR 0007 promoted it, this moves it onto the frozen interface. Modbus, BACnet, OPC-UA, SNMP, REST polling and DCS each still need **their own ADR** under §10 — unconditionally, not only where a protocol library has to be settled under §9.4; see §6. Adapters never read `process.env` (ADR 0016 §4); the host reads it in `host/config.ts`, **plus** the pilot-era `MQTT_*` and `CREDENTIAL_ENCRYPTION_KEY` reads in `rtu-config.js` — **no longer the unmodified pilot file since `E8.4` (2026-09-11): ADR 0062 decision 2 moved its credential *key selection* into `packages/shared/src/credential-keys.ts`, because the ingest carries its own AES-GCM implementation and two copies of a rotation window disagree about which key reads which row in a way two copies of a pinned version never could. The cipher call itself is untouched; `rtu-config.test.js` is deleted and its checks live in a type-checked `rtu-config.spec.ts` (ADR 0016 Amendment 6). `resolveMqttConnection` now also returns `credentialSource`, so a row that exists but yields no readable credential is reported as `"env"` rather than disguised as `"db"` — `source` still means only that a config row existed**. That `MQTT_USERNAME`/`MQTT_PASSWORD` fallback is the *only* working credential path, and ADR 0016 Resolved decision 5 expected it to **survive cutover**. **It did, and the expectation is no longer a prediction:** the pilot has run on that path since 2026-08-06, `bms.rtu_connection_configs` held no rows when the cutover ran, and the decision's own caveat — that the emptiness was measured on a local seeded database and needed confirming against the production pilot — is discharged by that database *being* the pilot's. Treat the emptiness as a **measurement with a date, not a standing fact**: the onboarding wizard writes that table, so re-query before relying on it. **ADR 0016 Amendment 3 (2026-08-14) now records this**, and it re-measured rather than restating: `bms.rtu_connection_configs` still held **0 rows**, so the fallback survives, and `CREDENTIAL_ENCRYPTION_KEY` *is* set — the ADR 0012 path is blocked on **data, not configuration**. Amendment 3 also names the repository owner as §6 commit 4's owner, closing Resolved decision 4. Writing an `rtu_connection_configs` row is still prerequisite work for anyone who wants the ADR 0012 path. Amendment 1 widens the schema fields to `ZodType<T, ZodTypeDef, unknown>` so `.default()`/`.transform()` schemas compile; Amendment 2 adds `@types/pg`. **§6 commits 3 and 4 are both discharged** — the parallel run and the cutover ran against the live PHE feed on 2026-08-06, and commit 4 landed 2026-08-14 (PR #30): `src/index.js` and the `INGEST_NOTIFY` flag are deleted, `pnpm start` is the host, and `pg_notify` is unconditional. That was not tidying up — post-cutover the flag's off-default was the only reachable state in which telemetry lands while every dashboard is dead, with no error and no alarm. **Four of commit 4's five actions landed. The fifth did not, and that is §6 being followed rather than amended**: it conditioned retiring the `MQTT_USERNAME`/`MQTT_PASSWORD` fallback on the pilot RTU having an `rtu_connection_configs` row, and it has none. Reassigned to **`E8.4`** |
...
| Secrets      | AES-256-GCM encrypted RTU connection credentials via `CREDENTIAL_ENCRYPTION_KEY`; never returned decrypted by the API (ADR 0012). Writers into that store: the master-data RTU admin, and the onboarding credentials endpoint above (**ADR 0022**), which **fails closed** with 503 when the key is unset rather than reporting a success that stored nothing. In an onboarding draft the blob is keyed by **RTU `code`, never by array position** — the draft's `rtus` array is replaced wholesale by any patch, so a positional key delivered one broker's password into a different broker's connection config. A code claimed by no RTU, or by more than one, drops rather than guesses. **SMTP stays platform-owned in the environment** (**ADR 0043** decision 13, reaffirming ADR 0041 decision 8): `SMTP_HOST`/`PORT`/`USER`/`PASSWORD`/`SECURE`/`FROM` are read once in `notifications.config.ts`, no service body reads `process.env` directly, and none of them appears in any UI. An organization owns presentation and routing only — from-name, reply-to, recipient lists, webhook URL, webhook HMAC secret and the per-channel rate limit, all in `notification_channels.config` and the encrypted secret columns above — never the transport itself. Per-organization SMTP relays stay deferred; the mechanism above (`secret_ciphertext`/`secret_iv`/`secret_key_version` plus `CredentialCryptoService`) is what a future per-org relay password would use, not what runs today. **Since `E8.4` (ADR 0062, 2026-09-11) the key is rotatable, and the two surfaces rotate together** — RTU credentials and channel secrets were always encrypted by one service, so a rotation covering one table would have left the other unrotatable and silently stale. `CREDENTIAL_ENCRYPTION_KEY_PREVIOUS` is accepted for **decryption only** at `CREDENTIAL_ENCRYPTION_KEY_VERSION` minus one; `decrypt` selects a key **by the version stored on the row** and refuses any other rather than trying both, because an AES-GCM tag check would make a guess *succeed* and a rotation that accepts an unknown version can never report that it has finished. Both applications refuse to boot on a dead window — a previous key at version 0, a previous key with no current key, or either key that is not 32 bytes (Amendment 1). `pnpm --filter api rotate-credentials` walks both tables as `bms_fleet`, selected on ciphertext rather than version. **Two carriers it does not walk, both recorded rather than hidden:** an onboarding draft's `_secrets` blob, which commit copies verbatim rather than re-encrypting, and which the runbook at `docs/security/encryption-at-rest.md` §3.1 step 5 handles by draining open drafts before the previous key is unset |
| Operations   | Work orders, maintenance schedules, basic rules, Energy CSV reports, completed 2D Control Room foundation screens, completed guided rule builder, and completed Control Room extension. **Since ADR 0076 (`F3.66`–`F3.70`) the Control Room is one sidebar entry**, not the *Control Room 2D* group: `/control-room` → `/control-room/org/:organizationId` → `/control-room/site/:locationId`, a level with one item skipped, every scope but `none`. A site with no `bms.site_control_room_views` row shows the generated view (`GET /api/v1/control-room/sites/:locationId/generated`); a `dashboard` view renders that dashboard inline (`F3.69`); and the SMOC site, `RSMOC-WC` in `ESKOM`, shows the seven SMOC pages for its `builtin` view as tabs at `/control-room/site/:locationId/:tab?` (`F3.70`). The old `/cr-*` routes redirect to those tabs; the `F4.156` route gate is gone, and access is "can read the site" plus the per-area asset-group rule on each tab. Every mutating endpoint across these four domains is gated by the **operations write matrix** (ADR 0017) — see §4.7. On the **read** side, `GET /rules/executions` is the one route here with no role gate of its own — it scopes on `readableAssetIds` alone, so `operator`, `viewer`, `location_admin` and `asset_group_admin` all reach it. Since **ADR 0046 Amendment 3** (`E8.6`) its `trace.evaluatedBy` — the evaluating operator's IdP subject — is removed for every non-`admin` reader, in SQL and keyed on the database role; the writers still record it, because the global admin's view is the forensic record. The Energy CSV export escapes through the shared serialiser, **not** its own rule — see the *CSV exports* row (ADR 0026) |
...
| Notifications | **ADR 0041** (`F3.8`): one `NotificationTransport` interface in `apps/api/src/notifications/`, with `log`, `email` (`nodemailer`) and `webhook` implementations. **Dispatch is inline and fire-and-forget — no queue, no Redis** — and `dispatch()` **never rejects**: a failure is a `failed` delivery row, not an exception thrown back into the alarm path that raised it. **A row is written for every attempt, including the ones that send nothing — with three exceptions, all on the event path, and since `F3.54` all three ask one question.** Two are failed reads (plan D3's ledger read, security review H1's rate-limit read); the third is a decision, `F3.48`'s hourly ceiling (2026-09-08, ADR 0057 Amendment 2 and ADR 0041 Amendment 3). At each of them, **a dispatch the sweep will offer again on its own writes nothing** — an escalation step — because the row would spend that event's dedupe key for the life of the ledger and the key must survive for a later tick to retry it. **A refused cleared message keeps its row at all three** (ruling Q-A for the ceiling; `F3.54`, 2026-09-08, ADR 0057 Amendment 4 and ADR 0041 Amendment 4 for the two reads): it is dispatched once from the clear phase and never re-offered, so silence buys no retry and costs the only evidence. The three exits call one **exhaustive** predicate rather than testing the kind inline, so a third, retried event kind is a compile error and not a silent behaviour change. The raise path is unchanged at these three — though its own `F3.46` transition dedupe writes nothing once a refusal for that key is recorded, which is a different case with a different reason. **Since `F3.51` (2026-09-09, ADR 0041 Amendment 5) a raise that wrote one of those three refusal rows is no longer lost for the life of the alarm**: the alarm lifecycle sweep re-offers the same `DispatchInput` on a later tick, the body's age clause aside (`F3.57`; see *Alarm lifecycle* below), so `dispatch()` itself is unchanged but `record()` now reports whether its ledger row landed (`rowLost` on `DispatchOutcome`) — a failed insert leaves neither bound above with anything to count, so the caller, not this service, is what stops the retry. The **six** statuses are `sent`, `failed`, `skipped_unconfigured`, `skipped_deduped`, `skipped_rate_limited` and — since `F3.52` (migration `0068`) — `skipped_stale`, for a due escalation step abandoned more than 60 minutes past its due instant rather than sent late; because *"no notification arrived"* and *"no notification was attempted"* are different answers to an operator and only the ledger can tell them apart. **A status says what happened to an attempt; since `F3.56` (2026-09-10, ADR 0041 Amendment 8) a sixth DTO-only field says what the attempt was FOR.** `NotificationDeliveryDto.event` is `raise`, `escalation`, `cleared`, `test` or `unknown`, **derived** in `listDeliveries`'s `.map()` by `parseDeliveryEvent` (`dedupe-key.ts`, beside the writer) and never stored — decision 9's "the kind lives in the dedupe key, not in a new column" is unchanged, and `tests/adr-0041-notification-invariants.test.ts` deliberately does **not** mirror this enum, because it compares the status set against a database CHECK and the event set has none. The raw key is selected and dropped in that same `.map()`: it carries a rule uuid, an alarm uuid and the **severity code**, and the severity is the incremental exposure, the other two already being DTO fields. Two things about the parse are load-bearing. It **prefix-strips** the row's own rule and alarm rather than counting colons, because `alarm_severities.code` is an open vocabulary with no format CHECK — a colon in a code defeats a segment count, and a code named `cleared` defeats a bare suffix match, since `rule:alarm:cleared` is a *raise* of a `cleared`-severity alarm. And **`no-alarm` in a key does not mean a send test**: `F3.46`'s transition refusal writes that form for a raise with a null `alarm_id`, so only a NULL key is a test — the totality claim rests on enumerating `record()`'s nine call sites, not on any row count. Storm control is two-sided: a transition dedupe (an unchanged plant sends nothing) and an hourly ceiling that counts `sent` **only**, so throttling cannot throttle itself. **A channel's secret never touches `config`** — `config` is returned by the API and appears in logs (§9.6), so the credential lives in three columns holding ADR 0012 ciphertext, IV and key version, made all-or-nothing by a CHECK; the DTO carries `hasSecret: boolean` and never the value. **The kind vocabulary is a lookup table, not an enum** (ADR 0031 A1), so `F3.9`'s `sms` is an `INSERT` — but it is **not yet open end-to-end**: the admin UI hardcodes two `<option>`s and the transport lookup is a `switch`, so a new row still needs code. `notification_deliveries` FKs are `NO ACTION` on purpose — history outlives configuration, and deleting a channel with history returns a **409 telling the operator to disable it instead**, found by clicking the button rather than by the compiler. **Webhook egress is restricted at the transport**: loopback, private, link-local and this-network destinations are refused *before* `fetch`, with `redirect: "manual"` and a 5 s timeout. The IPv4-mapped IPv6 form (`https://[::ffff:7f00:1]/`) reached the Compose network until the security review caught it — the unit tests had asserted the dotted form, which `new URL()` never produces, so they were exercising an unreachable branch. The residual DNS-rebinding window is documented in the file header, not implied. **Since `F3.46` (2026-09-06, ADR 0041 Amendment 2), a refusal is recorded once per channel, organization and dedupe key** — `dispatchToChannel` reads the ledger before writing `skipped_deduped`, not once per press, and migration `0065` carries the first index on `dedupe_key`. Both `/rules/:id/notifications` routes carry **role AND scope** checks per §4.7; the role gate alone admits `location_admin`, which let a scoped admin redirect or silence another site's alarms until review found it. **`F3.7` (2026-09-06, ADR 0041 Amendment 1) added the two production callers of `dispatch`, and they are deliberately asymmetric**: `RulesService.evaluateEnabledRules` dispatches on every attempted raise, so a repeat *Evaluate now* records `skipped_deduped`; `AlarmEngineService` dispatches only on a transition, because it re-observes every open alarm five times a minute. `rules/rule-actions.ts` is the **one** reader of a rule's stored `action` — add a meaning for `review` there or nowhere; today it is inert. The sweep's dispatch is cross-organization as the sweep itself is (ADR 0033 decision 2), by owner ruling. `setRuleChannels` keeps joins to channels outside an org-scoped caller's manage scope (a fleet-global channel is hidden from `list()` but present in the join) and audits `preservedChannelIds`; the rules card's per-rule picker carries those hidden ids through. **No file in this area now sits within 15 lines of §4.5's 1000-line cap**, measured 2026-09-10 after `9a577afe`. `rules.service.ts` left the list at **972**, and it left the way the list asks: `F3.49` moved `assertCompatiblePoint` to `rule-points.ts` in its own commit **before** the feature added a line — 990 → 970, then 972 once the feature and its constructor comment landed — gated by `git diff -w` showing only the signature, `this.fleetDb` → `fleetDb`, and the import and call lines. That is three consecutive rows leaving this list the intended way. **The files nearest the cap are outside this area**, and the first draft of this sentence named the wrong one — which is precisely the "read as permission" failure the warning above describes, committed in the entry that carries the warning. Measured on the merged tree: `stock-catalog/stock-catalog.spec.ts` at **999**, one line under the hook; `asset-templates-content.schema.spec.ts` at 998; `onboarding-chat.service.spec.ts` at 996; `asset-templates.lifecycle.integration.spec.ts` at 995; and only then `asset-templates.seed-rules.integration.spec.ts` at **993**, which `F3.49` moved from 989 after the §4.5 hook refused an append there at 1120, sending its new picker cases to a sibling spec. Extract before adding to any of those five, and **re-measure rather than trusting this list** — it is a snapshot of 2026-09-10, not a fact about the repository. **`channels.service.ts` has left this list, at 964**, and it left the way the list asks: `F3.56` found it at 989, and `readiness()` moved to `readiness.ts` (78 lines) in its own commit **before** the feature added a line, gated by `diff -w` against the base bytes showing only the signature and `this.fleetDb` → `fleetDb`. Two things about that method made it the cheap one to move and are worth reading before picking a different target next time: it was the sole user of the file's module-level `sqlCount` and of its `isNotNull` import, so the move took both out with it, and a four-line delegate kept its exact signature, so not one of its six callers changed. `F3.52` took `notifications.service.ts` to **999** and then extracted twice (`ledger-text.ts`, `dispatch-shapes.ts`), which is the shape this list is asking for: one line under the cap is legal and is not headroom. **`notifications.service.ts` has now left this list too**, at **833**: `F3.53` needed 14 lines in a file that had 18, so it moved the two dedupe-key ledger reads to `ledger-reads.ts` **before** adding them — the order the list is asking for, not a rescue after the fact. Its move gate was weaker than `F3.52`'s and said so: nothing was left at module level in that file, so every candidate was a method, and the claim checked by `diff -w` was *identical apart from indentation, the signature and `this.fleetDb` → `db`* rather than byte-identity. **`alarm-lifecycle.service.ts` left this list earlier**: it reached 991 under `F3.55` and `4a2d00ec` moved its three phase bodies to `alarm-lifecycle-phases.ts`, leaving **503 and 879** — re-measured at `F3.60`, which batched the rule-channel read and added the phase's pre-pass; the figure has now been re-measured at `F3.53`, `F3.57`, `F3.59` and `F3.60` and read 720, then 728, then 772, then 795 mid-branch before settling at 879, so **re-measure it rather than trust it** — this row has never once been read correctly from the page. That move is the shape the rest of this list wants — a pure move, gated by a diff showing only the `export` keywords the extracted entry points needed, not by a green suite. **Re-measure rather than trusting these figures**; the entry said 994 for `rules.service.ts` for some weeks after it had dropped to 990, and a stale number here is worse than none, because it is read as permission. **The ledger has a read rule as well as a write rule now:** which statuses answer an event's dedupe key is decided in `eventDeliveryBlocked`'s SQL `WHERE`, never by filtering the rows it sampled — the read takes three rows with no `ORDER BY`, so an unordered sample filtered in TypeScript can leave both arms false and re-send an event that was already sent (`F3.48` Amendment 2 §3, `F3.50` Amendment 3 §3). The `F3.8` storm-control suite dispatches `raised: false` for **every enabled rule** in the seeded database, so a hand-made `rule_notifications` join left in place during a suite run collects one skip row per run. **`F3.47` (2026-09-09, ADR 0033 Consequences discharged) bounds the sweep `RulesService.evaluateEnabledRules` runs, not the table**: `POST /rules/evaluate` now asks an in-process `EvaluateThrottle` before it sweeps and answers 429 with `Retry-After` when the caller's bucket swept less than `LIFECYCLE_TICK_MS` ago, keyed on `readableOrganizationIds` with an unrestricted admin and a grantless `configuration`-role caller each in their own bucket rather than sharing one. The resulting ceiling is **K + 1 + G full cross-organization sweeps per 30 s per API process** — one per organization, one shared by every global admin, one per grantless principal — because decision 2 still makes each sweep ignore the caller's scope; `AlarmRaiseService`'s streaming-path writes and `POST /rules/preview`'s audit row stay unthrottled, and `F4.17` stays open. **An argument passed at a call site is gated by nothing unless a test observes it**: the controller's one call to `throttleKeysFor` passed the caller's identity, and every route test ran the branch where that argument is discarded, so replacing it with a literal, and later with `user.name` — which `bms.users` does not make unique — both typechecked and left the suite green until a test exercised the branch that reads it |
| Alarm lifecycle | **ADR 0057** (`F3.10`, 2026-09-07): an alarm is **active while `cleared_at IS NULL`**; acknowledgement is an annotation, never a closure (ISA-18.2), and `POST /alarms/:id/ack` never clears. The four derived states are active, acknowledged, cleared-unacknowledged (leaves the active rail, stays listed until acknowledged) and closed. `bms.alarms` carries `cleared_at` and `normal_since`; `bms.automation_rules.clear_hold_seconds` (1–86 400, `NULL` = 120 s) is the hold; `alarms_open_per_rule_uidx` is on `(asset_id, rule_id) WHERE cleared_at IS NULL AND rule_id IS NOT NULL` since `0066`, so an acknowledged alarm whose condition still holds does **not** re-raise and a new row opens only after a clear. **One `AlarmLifecycleService` sweep** — three files since `4a2d00ec`: `apps/api/src/alarms/alarm-lifecycle.ts` pure, `alarm-lifecycle-phases.ts` the three phase bodies, `.service.ts` the shell that holds `runLifecycleSweep`, the `AlarmLifecycleDeps` contract and the Nest wiring — is the third host of `runSweepLoop` (`apps/api/src/scheduling/sweep-loop.ts` — every sleep-loop sweep uses it; never `setInterval`), 30 s tick, in the API process: the clear phase reads the latest sample per open threshold alarm through `batchedLatestPointValues`, a stale sample changes nothing (ADR 0027), a match resets `normal_since`, a non-match past the hold stamps `cleared_at` inside `withTenant`, broadcasts `cleared` after commit and sends the cleared message **only to channels holding a `sent` row for that alarm**; the escalation phase selects active *and unacknowledged* alarms, maps organization and severity to a profile, and sends each step whose `after_minutes` have elapsed since `raised_at` once — idempotency is a ledger read on `notification_deliveries_channel_key_idx`, a `failed` row is retried up to `MAX_EVENT_ATTEMPTS` (3), and **since `F3.48` (2026-09-08) a step the hourly ceiling refused is retried too** — it writes no row at all, so the key survives, it never spends one of the three attempts, and nothing bounds the retry but the ceiling lifting; a `skipped_rate_limited` row written before `F3.48` no longer blocks a key either, which is what released the first mapping's stranded backlog. **Since `F3.50` (2026-09-08, ADR 0057 Amendment 3) a `skipped_unconfigured` row blocks only while it is NEWER than `max(channel.updated_at, PROCESS_STARTED_AT)`** — a step refused because its channel had no URL, no recipients, no readable secret or no transport at all is offered again once an operator reconfigures the channel or the API restarts. That row keeps being written, which is the whole difference from `F3.48`'s ceiling: a ceiling refusal is a self-clearing postponement, an unconfigured channel is a configuration fault an operator must see. The exclusion is `or(status <> 'skipped_unconfigured', attempted_at > watermark)` **inside** the `and`, never hoisted over the whole read — a watermark on the whole read would drop an old `failed` row out of the sample and reset the three-attempt cap on every channel edit. Any PATCH moves the watermark, a rename included (an accepted cost), and a fleet-global channel carries one watermark for every tenant while the ceiling is per-tenant. `sent` and `skipped_deduped` still block for ever. Each step under its own try/catch. **Escalation profiles** are four tenant tables under FORCE RLS — `bms.alarm_escalation_profiles`, `_steps` (`step_no`, `after_minutes`, strictly increasing, at most ten), `_step_channels` (at most 50 per step, organization-paired like `setRuleChannels`, a step with no channel refused), `_defaults` (`organization_id`, `severity` → `profile_id`) — administered by `GET/POST/PATCH/DELETE /admin/escalation-profiles` and `GET/PUT /admin/escalation-defaults` under the channel-admin gate, strict bodies, audited; no seed, nothing escalates until an operator maps a severity, and the first mapping sends the step to every backlogged alarm of that severity at once. Escalation ignores the rule's `action`. The two event kinds ride ADR 0041's dedupe key as `:escalation:<n>` and `:cleared`; no new column. Pages: the alarms page's **Active card counts uncleared alarms, acknowledged ones included** (a recorded divergence from the mockups' "Active (unack)"), Ack with a required reason on any unacknowledged row; the rules card carries *Clear hold (seconds)*; `/admin/escalation-profiles` follows the channels page pattern. Dashboard KPIs, map counts and the metric catalogue count active = not cleared. Out of scope until their rows: a per-rule profile override, a shelve action, the `/cr-overview` Active Alarms rail (`F3.28`, now buildable). **Since `F3.51` (2026-09-09, ADR 0057 Amendment 5) the sweep gains a third phase, `runRaiseRetryPhase`, between the clear phase and the escalation phase**: a still-active unacknowledged alarm's original raise — the same `DispatchInput` and the same dedupe key, except `message`, which since `F3.57` appends the alarm's age once a whole minute has passed (ADR 0041 Amendment 9) — is re-offered to exactly the channels `channelsOwedTheRaise` shows are still owed it, closing the gap where a raise's own refusal row answered `rule:alarm:severity` while the next evaluation's transition dedupe checked the different key `rule:no-alarm:severity` and never retried. The stop condition is `eventDeliveryBlocked`'s whole predicate, not a hand-rolled count; a channel with zero rows under the raise key is not owed at all; an acknowledged alarm is skipped because somebody is already on it (`F3.51` owner ruling 4 — this read "because the retry is the raise text verbatim" until `F3.57` made the body carry the alarm's age, and the skip never rested on that anyway). Both new phases share one bounded in-process `LostLedgerRows` set, keyed per phase, for the triples a failed ledger insert would otherwise re-offer without limit — a restart empties it, which costs one extra send per remembered pair rather than nothing. `loadRaiseAttempts` (`apps/api/src/notifications/raise-attempts.ts`) reads every ledger row under a batch of raise keys in one query, chunked at 500 refs so one tenant's alarm volume cannot disable the phase fleet-wide. **Since `F3.55` (2026-09-09, ADR 0057 Amendment 6) `notifyCleared`'s two reads each `warn` rather than stay silent when they answer empty** — no channel holding a `sent` row for the alarm, or every channel that did being disabled now — and neither writes a `notification_deliveries` row, because the first has no channel to attribute one to and the second would record an attempt never made; neither is a refusal in Amendment 4's sense. **The two warns must stay distinguishable and neither line may ever carry `alarm.message` or a channel id/config (§9.6)** — the first return's warn fires on *every* clear of a rule with no `rule_notifications` join, which is the common seeded and template-built shape (no seed and no `asset-templates-instantiate.service.ts` write joins one), not only on a raise that left no `sent` row; read the log line on the `sent`-row predicate it states, not as "a message was lost". A per-channel guard testing an `endsWith`/`includes` pair on a comma-joined suffix is measurably weaker than testing `includes` alone against the *same* joined line — measure a guard against the shape a future line will take, not only the shapes the current one takes. **Since `F3.53` (2026-09-10, ADR 0041 Amendment 7) the sweep carries one `ClosedCeilings` per tick**, created in `runLifecycleSweep` and dead when it returns, so a channel the hourly ceiling has already refused inside that tick is not asked again: measured on a deployed build with 42 due alarms over 3 channels, **126 dispatches a tick issued 3 ceiling reads instead of 126**. It remembers the **closed answer only** — a `false` is never cached, so nothing can be sent on the strength of memory — and it is threaded from the sweep rather than held on the service, so `notifyCleared`, `sendTest` and the fire-and-forget `dispatch()` still read the ledger every time; only `dispatchRememberingLostRows`, shared by the two re-offering phases, passes it. **A remembered `true` is not monotone and the docblock refuses to claim it is**: `since` is recomputed per call, so a channel at its limit can drop below it mid-tick, and the postponement is one tick *unless* the alarm is acknowledged or cleared in that window (the raise path) or the step crosses `NOTIFY_STEP_MAX_LATENESS_MINUTES` and is abandoned as `skipped_stale` (the escalation path, because `stepIsTooLate` is checked *after* the ceiling). Both exposures were found by review, the second in the sentence written to correct the first. **(`F3.59`, 2026-09-10, Amendment 9) the raise-retry phase reads a rule's channels only for an alarm that holds evidence, and (**Amendment 10**, `F3.60`, with ADR 0041 Amendment 10) it reads every evidenced rule's channels in ONE batched round trip rather than one per rule — the per-rule memo is gone, a distinct-id `Set` does its collapsing, and a rule whose batch did not return is decided about in no way this tick.** The organization filter Amendment 5 put on `channelsOwedTheRaise`'s `rows` argument is **hoisted above** `loadRuleChannels` and the predicate is handed that very array, so the skip is behaviour-preserving *for the owed set* by construction rather than by argument — stage 1 answers `false` for every channel with zero rows, so an empty group is "not owed" for **any** channel list and the round trip could not change the answer. **Measured on the running stack: 78 channel reads a tick became none** (`pg_stat_all_tables` scan deltas, +234 over three ticks before against 0 over four after, with the ledger and alarm deltas as the controls that the sweep was alive). **Not** behaviour-preserving for the warn stream, and the qualifier is load-bearing: a no-evidence candidate no longer enters the per-alarm `catch`, so a rejecting `loadRuleChannels` warns none where it warned one per candidate. **Two traps for anyone editing this guard.** It filters the **ref's** organization — the rule's, which is what every delivery row is stamped with — and never the alarm's; and **the status exclusions and the unconfigured watermark must stay inside the predicate**, because a row that is `skipped_rate_limited` or staled-out `skipped_unconfigured` is still *evidence* even though it is not *eligible*. Hoisting either exclusion up to the guard drops a row the predicate must see and silently un-fixes the third of Amendment 5's three cases; before `F3.59` nothing in the repository gated that, and R20 in `alarm-lifecycle-raise-retry-evidence-guard.spec.ts` now does, which is why its fixture row carries that status on purpose. **Batching the reads that remain is `F3.60`, not this** — a different defect on the same line, declined here by owner ruling. One bookkeeping gap is named rather than folded in silently: `F3.57`'s substance is in this row already, cited to **ADR 0041** Amendment 9 above, but its **ADR 0057 Amendment 8** label — the raise-retry phase gaining the tick's clock as a required `RaiseRetryPhaseInput.now` — is not written anywhere here. That is a missing citation and not a missing promotion, so §10.1's one-promotion-per-sweep rule is not in play |
| Alarm philosophy surfaces | **ADR 0059** (`E2.2`, 2026-09-08): the four `content.alarms[].philosophy` fields reach two readers. `GET /api/v1/alarms/:id/details` returns a nullable `classPhilosophy`, resolved by **exact provenance only** — `automation_rules.source_template_id` plus `source_alarm_code`, two columns not three, since a row in `asset_templates` **is** a version — and **no `point_key` fallback**, because one point key carries several alarms. `GET /api/v1/alarm-kb` lists the **current published version per organization-and-code** and is readable by anyone who can see alarms, `viewer` included (ruling Q0b): deliberately **not** `isMasterDataRole`, which is what keeps the operator out of the authoring screen. Both reads run on `bms_fleet` (BYPASSRLS), so **each carries a hand-written organization predicate** and a mutation-checked test; delete either and its test reddens. Both are **reads** — nothing prefills the instance enrichment and there is no copy control (ruling Q1), because ADR 0034's class-versus-instance line survives in the schema only if it survives on the panel. |
| Configurable dashboards | **ADR 0047** (`F3.1`), and **all five children are on `main` since 2026-08-30**. Three tenant-scoped tables — `bms.dashboards`, `bms.dashboard_widgets`, `bms.dashboard_widget_points` — created by migration `0050` with `organization_id NOT NULL`, a `tenant_isolation` policy and `FORCE ROW LEVEL SECURITY` from the first migration, per ADR 0043/0045. **The third table is the point of the design**: a widget binds live `bms.asset_points` rows through a real foreign key rather than through keys in a `jsonb` blob, which is what ADR 0019 had to hand-build an orphan check for. **Each policy checks its org-bearing parents, not only its own `organization_id` column** — Postgres runs a referential-integrity check with row security **off**, so a foreign key never consults the parent's policy, and a correctly-stamped row could otherwise bind another tenant's point (proved on the running stack during `F3.1a`'s review, and closed the way `bms.asset_group_members` in migration `0047` §3c already did it). **`widgetType` is a closed `z.enum` + `CHECK`, deliberately the opposite of ADR 0031/0032** — see §4.8, which carries the reasoning. The vocabulary and the per-type `config` discriminated union live in `packages/shared/src/contracts/dashboard-builder.ts`; the drizzle declarations in `packages/db/src/schema/dashboard-schema.ts` (its own file because `bms-schema.ts` was near the §4.5 line limit). **All three tables now have a writer and a reader**: `F3.1b` owns the API in `apps/api/src/dashboard-builder/`, `F3.1c` the four renderers, and `F3.1d` the builder surface plus the read-only viewer route beside it (Amendment 4). This sentence read *"nothing reads or writes these three tables yet"* until 2026-08-30; the `F3.1b`/`F3.1c` sweep did not correct it, which is worth recording, because a §2 row that lags by two rows reads as current rather than as stale. **Two rules `F3.1d` established that a later change must not lose.** (1) **`update()` authorizes the *stored* scope before the destination scope, and both run ahead of the "both scope columns set" 400.** Reverse the two authorizations and a `location_admin` re-homes an organization-wide dashboard under its own site and then deletes it, because `remove()` reads the scope the PATCH just wrote; move either behind the 400 and that 400 becomes a cross-tenant existence oracle. (2) **The 12-column canvas is `DASHBOARD_GRID` in `packages/shared`, never restated in TypeScript.** Seven TypeScript modules import it and migration `0050`'s `dashboard_widgets_grid_bounds_check` is the only legitimate restatement, because SQL has no imports; `tests/f3.1d-grid-bounds-single-source.test.ts` scans `packages/shared/src`, `apps/api/src` and `apps/web/src` and fails **any** line that pairs a grid-axis token with a bare `11`, `12` or `24`. The count that rule guards was undercounted **three times** before the scan existed — three sites in the brief, a fourth in the plan, a fifth and sixth in the build, a seventh on the scan's first run — so treat a new grid literal as a defect, not a convenience. **`F3.1e` is done** (2026-08-29), and it touched the *template* side rather than these tables: it added the sixth authoring tab (ADR 0038 Amendment 4) and tightened the template widget arms so **each takes `WIDGET_POINT_CARDINALITY[type]` instead of one shared cap** ([ADR 0047](docs/adr/0047-configurable-dashboards.md) Amendment 3) — a gauge, tank and tile bind exactly one point, a chart up to `MAX_WIDGET_POINTS`. **Read the numbers from `@bms/shared`; never restate one.** The `MAX_WIDGET_POINT_KEYS` alias is deleted, so `noUnusedLocals` fails a re-introduction. Amendment 3 also records that **no stored content needed migrating** and why, and keeps that finding falsifiable: a widget failing a tightened arm **in stored `asset_templates.content`** returns the ruling to the owner rather than licensing a wider bound. **`F3.35` closed 2026-08-31 under [ADR 0048](docs/adr/0048-dashboard-metric-catalog-and-table-widget.md)** and added the fourth table, the fifth widget type and a second binding kind. `bms.dashboard_widget_sources` holds a widget's **catalog** bindings; `bms.dashboard_widget_points` is untouched, because **a catalog key is a foreign key to nothing** — the catalog is code — and a fourth table says so rather than making `point_id` nullable, which would leave a `NULL` meaning either "a catalog binding" or "a bug". A widget binds **exactly one kind**, enforced on the write path as a cross-field rule between the two arrays, never by a per-type count. **Two records decide which pairing is legal and both are read by the builder AND the API**: `WIDGET_SOURCE_CARDINALITY` counts and `WIDGET_SOURCE_SHAPES` types, and the count alone was not enough — a `value_tile` accepts one source, which a *dataset* satisfies exactly as well as a *metric*, so `alarms.active` passed every bound onto a tile and rendered blank with nothing thrown. **Migration `0055` widened `dashboard_widgets_widget_type_check` with `DROP` plus `ADD`, not an `IF NOT EXISTS` guard**: the constraint already existed carrying `0050`'s four values, so an existence check finds it and skips the widening while reporting success. Its journal `when` is hand-stamped above `0054`'s, which itself runs ahead of the wall clock — a real-clock stamp sorts before it and is silently skipped wherever `0054` is applied. **No npm dependency**: ADR 0048's one open §9.4 gate, a grid library, closed as *not needed* — a six-row card with a column picker is a `<table>`, the way `F3.1d`'s canvas turned out to be Pointer Events. **`F3.2` closed 2026-09-17 under [ADR 0067](docs/adr/0067-per-asset-default-dashboards.md)** (PR #463) and gave `bms.dashboards` a **fourth scope and a second stamp**: `asset_id` (cascade — a default dashboard is *about* one asset) and `asset_template_id` (the asset-template version row, no cascade, mirroring `template_id`), migration `0073`; `dashboards_scope_check` is now a **count** of the three scope columns `<= 1`, two stamp checks keep a stamp on one template and on an asset, and `tenant_isolation` was re-created with **five** parent legs in both halves — the `0056` lesson again. Every site that enumerated two scope axes now counts three: the two bodies, `DashboardsService.update`'s merged-row guard, and `canManageDashboard`, whose asset arm reads the asset row once and proves its organization and its location's organization explicitly (a `location_admin` manages through the asset's location, an `asset_group_admin` through membership). **Only instantiation writes the stamp**: the bodies refuse `assetTemplateId`, and a PATCH that moves or clears `assetId` drops it, because provenance does not survive a move. `AssetDashboardsInstantiateService` materialises every view of the pinned version inside the asset-instantiation transaction after the rule seed (ADR 0058 decision 9's position), and `POST /admin/asset-templates/:id/default-dashboards` backfills pinned assets in **chunked, resumable** transactions, skipping the ones already stamped. Widgets copy their own grid and config; keys resolve against the asset's own active points with an explicit organization predicate; a hole is a reported `partial`, never a refusal; a repeated key binds once. Views are written in code-point order of their name — `content` is `jsonb`, so record order is not a property the store has. All 27 stock classes carry an `overview` view, gated at build time. Three residuals are recorded in the row: the skip set keys on the stamp, so a dashboard whose asset scope was cleared and restored by PATCH collides on its slug at the next backfill; `asset-templates-instantiate.service.ts` sits at 996 of 1000 lines — extract before you add; and every stock entry ships `sourceDataKeyPattern: null`, so a stock template needs patterns on its draft before it instantiates. **ADR 0067 Amendment 1 (2026-09-17, the post-merge sweep, PR #465)** closed the first residual and a false gate: on the backfill a slug collision is a **per-asset outcome** (`skipped_slug_conflict`, one savepoint per asset inside the chunk transaction, nothing else in the chunk affected) rather than a stop, and a stock tile binds only a **required** measured, non-manual point — thirteen optional featured keys and four optional chart series left nine views, and the build-time gate now asserts `required`. **`F3.31` closed 2026-09-17 under [ADR 0068](docs/adr/0068-operator-assets-browser.md)** (PR #466) and gave the asset scope its operator path: `GET /dashboards?assetId=` narrows within `readableOrganizationIds` (an out-of-scope id answers `[]`, never 403), and the `/asset-browser` panel lists an asset's default dashboards as `/dashboards/<slug>?organizationId=` links — the same query the dashboards page puts on its own links, because one slug can live in two organizations on the fleet pool **`F3.43` closed 2026-09-18 under [ADR 0069](docs/adr/0069-chart-legend-asset-label.md)** (PR #486): the point binding DTO carries `assetCode` from the join `resolveBoundPoints` already makes, and every chart series is named `<assetCode> · <pointKey>`; the instantiate read-back is the DTO's second producer and resolves through the same function **`E4.2` closed 2026-09-23 under [ADR 0072](docs/adr/0072-sustainability-and-benchmarking-dashboards.md)** (PRs #519, #521; sweeps #520, #522): a catalog entry may now carry fields on its write schema, and `dashboard_widget_sources.params` — written since ADR 0047, read by nothing — is what `sustainability.total` and `sustainability.by_location` resolve against (`{ pointKey, aggregate }`, migration `0079` for the `catalog_key` CHECK `0054` had frozen). Both answer `coverage { fresh, carrying }` beside the value, so a tile says how many assets it actually read; the picker hides any entry declaring `params`, so it can never offer a binding the API refuses, and the binding write checks every `pointKey` against the active catalog. `GET /dashboards?section=` filters by the instantiating template's section through a **LEFT** join — an inner one would drop every hand-built dashboard (`template_id IS NULL`) from the unfiltered list — with `dashboard_templates.organization_id = dashboards.organization_id` inside the JOIN, which is the only container on the `bms_fleet` branch. A **role-free** section template instantiates organization-wide (`location_id`, `asset_group_id` and `asset_id` all `NULL`, ADR 0072 decision 1 amending ADR 0049 decision 4); one that binds a role still needs a group, and the organization-admin arm of `canManageDashboard` is what a `location_admin` fails. **`E4.3` closed 2026-09-24 under [ADR 0073](docs/adr/0073-water-balance-and-wastewater-recovery.md)** (PRs #531, #534, #536; sweeps #533, #535, #537): both `sustainability.*` entries take an optional `balanceRole` that narrows the carrying set and its coverage to assets whose `bms.assets.water_balance_role` equals it, and `water.balance` is the eighth catalog key (migration `0081`, the `0079` pattern), a dataset over `{ period }` with one row per site owning an `intake`, `reuse` or `discharge` asset — an `internal` asset alone makes no row, so this is **not** the `by_location` row rule. The binding write's `pointKey` check above is narrower since #533: `PUT /dashboards/:id/widgets` checks a submitted `pointKey` or `balanceRole` against live rows only when the dashboard's stored sources do not already carry it, inside the widget transaction, because the builder re-sends stored params and one retired value blocked every later save; template publish stays strict. A role-less `kl_*` total still sums every stage: the presentation description, the write schema's `.describe()` and the template `WidgetEditor` say so, and the builder's `WidgetInspector` does not; a role-less `outlet_kl_*` total sums reuse, discharge and internal outlets with no note anywhere — both gaps are `F4.152` |
...
| Object storage | S3 API through `@aws-sdk/client-s3` (ADR 0066, `F3.3`), MinIO in compose (`pgsty/minio:RELEASE.2026-08-04T00-00-00Z`, pinned — moved off `quay.io/minio/minio` on 2026-09-24, ADR 0066 Amendment 5, after MinIO's own `quay.io` repositories started answering 401; host ports on loopback only; `core`/`pilot`/`phe`/`realtime-smoke`), one bucket, keys `org/<org>/assets/<asset>/<image>` built server-side only. `bms.asset_images` (migration `0072`, FORCE RLS, policy checks the parent asset) is the authority: an object without a row is never served. Reads are **API-proxied** under the JWT guard and `canReadAsset`; `GET /health` carries `storage` on the API and not on the worker. Unset `OBJECT_STORAGE_ENDPOINT` = unconfigured (routes answer 503), half-configured = boot refusal, `http://` refused without `OBJECT_STORAGE_ALLOW_INSECURE=true`. SSE/TLS are deployer requirements (decision 8). See §4 rule 13 for the deferrals. |
| Reports | Three formats of one report from one row source (`energyTable`, ADR 0026 Amendment 2 for the XLSX beside the CSV; **ADR 0071**, `F3.5a`, for the PDF): `pdfmake@0.3.11`, a singleton (`setFonts`/`createPdf`, not the `PdfPrinter` class the ADR first named), the four Standard-14 Helvetica names behind a local-access policy that allows exactly those and a URL policy that denies every URL — no font file, no VFS, and ANSI only, so money is `<ISO code> <amount>` and a non-WinAnsi asset name renders the fallback glyph. `bms.report_files` (migration `0077`, FORCE RLS, own-column policy, no GRANT — `0041`’s default privileges) is the authority for a saved report: `location_ids uuid[]` is the readers’ scope snapshot (`{}` = whole organization), `object_key` is unique and never echoed, `delivery_status` is written by the render job (`none` → `sent`, `skipped_unconfigured` or `failed`; `delivery_error` carries counts and `err.name`, never an address). The save (`POST /api/v1/reports/energy/files`) takes an optional `organizationId` (required for the global admin and a multi-organization admin), refuses a calendar-invalid date at the parse, pre-checks the cap on the fleet handle, renders under `readableAssetIdsInOrganization`, puts the object, then inserts the row under the tenant GUC behind `pg_advisory_xact_lock` with the authoritative count; a failed row discards the object only after a fleet re-read proves the row absent. Readers: `AccessControlService.canReadReportFile` — global admin, an organization admin of the row’s organization, or a location admin holding every id in `location_ids` (empty needs organization rights); `asset_group_admin` is 403 with the master-data sentence before any read. The download is API-proxied with five headers set before `pipeline`; the delete is row-then-object. `GET /reports/files` answers a 200-shape with MinIO stopped and 503 when storage is unconfigured; `export.pdf` needs no store. The three export routes keep the silent date roll (recorded, ADR 0071 Amendment 1 item 7). **`F3.5b` (ADR 0071 Amendment 2, PR #513):** `bms.report_schedules` (migration `0078`, FORCE RLS, own-column policy; `report_files.schedule_id` with the partial unique `(schedule_id, period_end, format)`) is the authority for a schedule; `reports/report-period.ts` computes the next run and the previous complete unit in the schedule’s zone over `Intl.DateTimeFormat` (weekly runs Monday, monthly day 1; a DST gap shifts forward, a fold takes the earlier occurrence; the timezone rule is slash + constructs + segment-casing, because ICU canonicalises `Asia/Kolkata` to `Asia/Calcutta`); the `reports-dispatch` queue (fleet, repeatable every `REPORT_DISPATCH_INTERVAL_MS`, claims due rows `FOR UPDATE SKIP LOCKED` with `LIMIT 200`, enqueues one `reports-render` job per row with id `<scheduleId>_<periodEnd>` — a colon is refused — then advances `next_run_at`; a poison row is warned by id and left due) and the `reports-render` queue (tenant; the schedule and its assets read under RLS, one row and object per format, a format whose unique row exists is skipped, prune to `REPORT_RETENTION_PER_SCHEDULE` clamped to the format count, then — after the commit, through `runProcessor`’s continuation — the pruned objects go and the email is sent with the files read back from the store under `REPORT_EMAIL_MAX_BYTES`). `ReportsCoreModule` is loop-free (it provides `CalcParametersService`, never imports `CalcModule`) and is imported by both `ReportsModule` and `WorkerModule`; the worker imports `StorageModule`. Routes `GET` and `POST /api/v1/reports/schedules`, `GET`, `PATCH` and `DELETE /api/v1/reports/schedules/:id`: manage rights over every named location (`[]` needs organization rights), `canManageNotificationChannel` on `channel_id` and the channel must be `email` of the same organization, `MAX_REPORT_SCHEDULES_PER_ORGANIZATION` (50) under an advisory lock, a PATCH to `cadence`/`runAtLocal`/`timezone` or a re-enable recomputes `next_run_at` from now, DELETE removes the files row-then-object first (the FK is NO ACTION). Audit payloads carry ids and enums, never the name; the filename is built from two dates and a format only. |
...
│   │                            each behind ControlRoomScopeRoute in
│   │                            components/control-room-scope-route.tsx. Their
│   │                            pure logic: lib/control-room-levels.ts (the
│   │                            level skip, the crumbs), lib/smoc-pages.ts (the
│   │                            SMOC_TABS table, isSmocSite and the tab
│   │                            helpers), lib/site-view-notice.ts;
│   │                            api/control-room.ts is the F3.67 resolve read.
│   │                            The generated body (F3.68) is GeneratedSiteView
│   │                            and GeneratedSiteAssetCard under
│   │                            components/control-room/, over
│   │                            api/generated-site-view.ts,
│   │                            hooks/use-site-live-readings.ts and
│   │                            lib/generated-site-view.ts. The dashboard body
│   │                            (F3.69) is components/control-room/
│   │                            site-dashboard-view.tsx over components/
│   │                            dashboards/dashboard-live-canvas.tsx, which the
│   │                            dashboard viewer shares. The SMOC body (F3.70)
│   │                            is components/control-room/smoc-site-view.tsx,
│   │                            hosting the seven tab contents in
│   │                            components/control-room/smoc/ (overview, sld,
│   │                            ups, battery, hvac, env, it). The seven /cr-*
│   │                            routes are redirects through
│   │                            components/smoc-legacy-redirect.tsx.
...
│   │                            src/admin/asset-templates/. It is its own module
│   │                            for the reason ADR 0034 gave AlarmDetailsService
│   │                            its own file: the admin service authorizes as
│   │                            master data, and this one must not
...
│   │                            PUT lives on LocationsAdminController. Since
│   │                            F3.68 it also holds GeneratedSiteViewService
│   │                            and its controller, GET control-room/sites/
│   │                            :locationId/generated, whose latest-value
│   │                            window is the literal GENERATED_LATEST_WINDOW_SQL
│   │                            (7 days; tests/f3.68-generated-site-view.test.ts
│   │                            T10 pins the literal and its one use).
...
│   │                            src/contracts/generated-site-view.ts is the
...
### 4.2 React
- Functional components only. **One *exported* component per file** — a file's
  public surface is one component, and anything else in it is private to that
  file.

  This sentence used to read "one component per file, with one standing
  exception granted by ADR 0028". **That was measurably false and was corrected
  on 2026-09-05**: 21 files under `apps/web/src` hold more than one component,
  and `control-room-overview-page.tsx` holds 17 (a 2026-09-05 count; `F3.70`
  moved that file to `components/control-room/smoc/overview.tsx` and removed
  its page wrapper). A rule the codebase breaks
  twenty-one times is not a rule, and leaving it written that way teaches every
  reader to discount the section.

  What the code actually does, and what to keep doing: a page or feature
  component owns its file, and the small presentational pieces only it uses live
  beside it rather than in `components/`. Extract one when a **second** file
  needs it, or when it grows its own state or tests. Two cases where keeping
  them together is the point rather than a concession:
  `apps/web/src/components/static-value.tsx` holds `StaticValue` and
  `StaticTspan` — the HTML and SVG renderings of one concept, which always
  change together, so splitting them would put two halves of one decision in
  different files (ADR 0028); and a status pill defined next to the single table
  that renders it, which is why `asset-points-page.tsx` holds `CalcRuntimePill`
  (`F2.9`).
...
- **A platform-vocabulary table has no policy, so its GRANTS are its only
  ...
  database draws the line the ADR draws. **A new column re-opens the kept
  `INSERT`:** `F3.68`'s fleet-wide display rank `headline_rank` was writable by
  that tenant insert, so migration `0084` (**ADR 0076 Amendment 2**) adds a
  `BEFORE INSERT` trigger, `bms.point_keys_refuse_tenant_headline_rank()`,
  raising `42501` when `current_user` is `bms_tenant` and the rank is not
  null. **A trigger, not a column grant** (owner ruling): Drizzle names every
  column, unset ones as `DEFAULT`, and Postgres checks the privilege of every
  named column, so a grant that left the column out would refuse the whole
  onboarding insert — and would need re-granting for every later column. Its
  known limit: it names one role, so a future login role that inherits
  `bms_tenant` passes it and needs a follow-up migration. The other six still hold all four
...
- **You cannot instantiate a Nest module in a test here, and it is not worth
  ...
  gate.
- **A widened return type is invisible to two of this repo's three gates, and
  only `pnpm typecheck:tests` sees it.** `F3.51` widened `dispatch()`'s return
  to `DispatchOutcome[]` and left `rules/rule-actions.spec.ts` declaring the
  old `DeliveryResult[]` (`TS2322`) — CI was red while `pnpm build` (which
  excludes `*.spec.ts` via `tsconfig.build.json`), `pnpm test` (which strips
  types with esbuild) and `pnpm typecheck` (which never opens a spec file) were
  all green. Run `pnpm typecheck:tests` before believing a signature change is
  safe; its `apps/api` `tsc -p tsconfig.json --noEmit` leg is the one gate that
  compiles a spec against the real types, and running that leg alone is a
  complete enumeration of what a signature change broke.
...
**ONE read surface deliberately sits outside the master-data gate.**
`GET /api/v1/alarm-kb` (**ADR 0059** ruling Q0b, `E2.2`) serves organization-scoped
template content — the authored alarm philosophy — to **anyone who can see
alarms, `viewer` included**. It uses `JwtAuthGuard` plus
`readableOrganizationIds` and deliberately **no** `isMasterDataRole` and **no**
`assertOperationsWriteRole`. That is not an oversight and it is not a widening
that slipped through: a `viewer` already reads these same four fields on the
alarm details panel, so gating the KB would refuse in one place what the product
hands out in another — and the whole reason the row exists is that the operator
and the technician cannot open the template authoring screen. **The absence of
the gate is load-bearing**, so `tests/e2.2-alarm-kb-route-gate.test.ts` holds it
as an absence on both sides: the controller's missing role check, and the
`/alarm-kb` route in `app.tsx` carrying no `AdminRoute`. Adding either breaks no
other assertion while silently closing the surface, and it fails **open as an
empty list** rather than as a denial — the same shape `E7.1d` records for the
notification screens. `readableOrganizationIds` is the *only* control here, and
it runs on `bms_fleet` (BYPASSRLS), so `null` — and only `null` — may mean every
tenant.
...
it. Since **ADR 0076 Amendment 2** (`F3.68`, migration `0084`) that `INSERT`
may not carry a `headline_rank` either — a trigger refuses it with `42501`
(§4.4) — and the rank is a global-`admin` write on `/admin/point-keys` like
...
affordances `F3.28` delivered against the client's reference dashboards
(the Active Alarms rail, the breach value in alarm text, the KPI "vs
yesterday" deltas, the state legend, the Diagram/List toggle, the per-class
health strip, the Key Parameters gauges and the footer ribbon) — read it
...
- Template content sections whose consumer does not exist yet. **ADR 0019
  ...
  **`F3.1d` closed 2026-08-30 and the `F3.1` umbrella with it**, which discharges
  ...
  is `F3.2`. *(That sentence read "which is `⬜`" until 2026-09-17: `F3.2`
  closed under [ADR 0067](docs/adr/0067-per-asset-default-dashboards.md),
  PR #463, so the second consequence is discharged too — see the
  *Configurable dashboards* row in §2.)* ADR 0047 Amendment 4 drew the
  ...
  **Nothing in the ADR 0019 tier list is held open by `F3.1` any longer**;
  ...
  the other until `E1.3` reopened it on 2026-08-30 — see above.) ADR 0047 decision 6's boundary was unchanged by the closure: `F3.2`,
  ...
  **ADR 0055 shipped on 2026-09-05 (`F2.9`), and `F2.8` shipped on it on
  2026-09-06** — the stock incomer carries `site_kw`, `it_kw` and `pue` as
  `v2` points and the fitted PUE curve is gone from all three copies.
  Every guard the ADR repeals is still in place **for `v1`** and still refusing
  what it refuses — that is decision 3, and it is enforced at read time as well
  as at write time, because the failure that motivated it was a stored dialect
  label disagreeing with the formula beside it. A `v1`-labelled definition
  referencing a derived sibling is refused by the loader as
  `v1_references_derived`, whatever the label claims.
...
- Deploying template *maintenance* content into running objects. A maintenance
  plan does not become a `bms.maintenance_task_templates` row (its `asset_id` is
  `NOT NULL`); that wiring is `E3.x` work needing its own ADR. **The alarm half
  of this bullet is delivered** — `E2.4` (ADR 0058, 2026-09-07) makes every
  `content.alarms[]` entry become one `bms.automation_rules` row per created
  asset at instantiate, inside the same transaction. The three fields a template
  did not carry are supplied rather than invented: `ruleType` is always
  `threshold`, `condition` is the latest reading, and `action` is always
...
- ~~Energy reports (**PDF**).~~ **Promoted 2026-09-21 by ADR 0071 (`F3.5a`)**
  ...
  *Amendment 2*, `F4.51`) and the CSV. **Scheduled delivery landed 2026-09-22 (`F3.5b`, PR #513).** What stays out: a bundled Unicode font (the standard fonts are ANSI
...
to list both report items as out of scope. Scheduled report delivery **is in scope since ADR 0071 Amendment 2 (`F3.5b`, 2026-09-22)**. CR
...
and the transport `switch` is what holds them there. **Escalation and
auto-clear shipped 2026-09-07** — `F3.10`'s profiles and the lifecycle sweep
are ADR 0057 and §2 *Alarm lifecycle*, so this paragraph gates neither any
more; `F3.7` (making a rule's stored `notify` actually fire) **shipped
2026-09-06** — both raise paths dispatch, the streaming engine only on a
transition, the sweep cross-organization as the sweep itself is, and the rules
card carries a per-rule channel picker (ADR 0041 Amendment 1). **It promotes
no closed content section**: `health`,
