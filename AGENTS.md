...
> never the acting operator's `oidcSubject` (**ADR 0046** and its two
> amendments, `E7.1e` and `E7.1h`) — real rather than theatre only because of
...
> binds it (**ADR 0045** and its three amendments, `F4.16`/`E7.1a`).
> General
...
channel is ruled. **It is no longer the only instance, though — read the two
together.** ADR 0046 Amendment 2 (`E7.1h`, above) applies the same rule to a
second table on a different trigger: Amendment 6 fired because a *column*
carried fleet detail into a tenant's view, Amendment 2 because the *audience*
widened under a projection nobody re-read. Neither generalises on its own; what
generalises is the question — **when a scoped read's audience or join changes,
the `SELECT` list is reviewed with the `WHERE`.**
...
- **Multi-tenancy and row-level security are delivered** (**ADR 0043**,
  ...
  reader now sees `actorEmail` and never the subject — see §4.7. **Still
  deferred:** org-level **read** RBAC on
