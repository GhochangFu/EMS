...
had fallen behind that count before this sweep corrected it. The first two
...
- **Multi-tenancy and row-level security are delivered** (**ADR 0043**,
  ...
  `canManageNotificationChannel`. **Still deferred:** per-organization SMTP
  relays (decision 13 — the ADR 0012 credential-store mechanism already
  exists, not wired to it), white-label branding per tenant, a
  `platform_admin` rung above `admin` (decision 2 — two named `admin`
  accounts express the distinction instead of a new role), and `telemetry.*`
