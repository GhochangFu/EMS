...
| `CREDENTIAL_ENCRYPTION_KEY_PREVIOUS` | **Secret** — rotation only | unset (from `.env`) | Same rotation-window key as the API, decryption only (ADR 0062). Must match the API's value or the ingest cannot read a row rotated on one side but not the other. See [`security/encryption-at-rest.md`](./security/encryption-at-rest.md) §3. |
| `CREDENTIAL_ENCRYPTION_KEY_VERSION` | No | unset (default `1`) | Same version as the API's current key. See [`security/encryption-at-rest.md`](./security/encryption-at-rest.md) §3. |
...
| `INGEST_STALE_AFTER_MS` | No | `300000` | Silence longer than this marks one RTU `stale` on the health endpoint (`F1.7`). Five minutes is measured rather than chosen: the nine live PHE RTUs publish every ~60 s (probe, 2026-08-22, 600 s window — [`docs/f1.7-fleet-probe.md`](./f1.7-fleet-probe.md)), so it is five missed cycles and a single dropped message can never raise it. Widen it for a protocol that polls far more slowly than MQTT pushes. Plain decimal digits only: `1e21` is refused at startup rather than silently switching the alarm off. |
