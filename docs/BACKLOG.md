...
WAVE 1  F1.2 F1.3 F1.4 F1.5 F1.6 F1.7 F1.10  [F2.2 ✅] F2.4  F3.7 F3.10 F3.1 F3.4 F3.11
        F4.5 F4.7 F4.8 [F4.10 ✅] F4.14 F4.23 [F4.28 ✅] [F4.29 ✅] F4.30 F4.31 F4.32 F4.33 [F4.34 ✅] F4.35 F4.36 F4.37  [E1.7 ✅] E3.1 E5.4
...
| F4.36 | Validate the `bms_telemetry` NOTIFY payload before broadcasting it — **raised by the `F4.34` security review (2026-08-14), pre-existing since ADR 0002/0007 and not caused by that item.** `parseNotification` in `apps/api/src/telemetry/telemetry-listener.ts` does `JSON.parse` → `Array.isArray(payload.readings)` → an **unchecked `as TelemetryReading[]` cast** → `hub.emitReadings` → Socket.IO fan-out to every subscribed client. `F4.34` preserved that behaviour deliberately rather than changing a data path under a reconnect fix; the only deltas it made were logging the non-array case and never logging the payload itself. **The trust boundary is not the MQTT edge**: `NOTIFY` requires no table privilege, so any role that can connect to the database can publish to the channel, and the legitimate producer (`apps/ingest/.../normaliser.ts`, parameterised `SELECT pg_notify($1, $2)` after `COMMIT`) is only one possible writer. A malformed-but-array payload therefore reaches browsers as a `TelemetryReading` that is not one — the same class §4.3 requires Zod for on HTTP DTOs. Scope is one schema at the parse boundary plus a decision on what to do with a batch that partially validates (drop the batch, or drop the bad readings and deliver the rest — they are not equivalent for a monitoring product). Cheap, and it is the last unvalidated path into the realtime fan-out | P2 | 1 | 1 | — | ⬜ |
| F4.37 | A **future-dated** telemetry timestamp renders a dead asset as `running` — **raised by the `F4.36` security review (2026-08-14), pre-existing and only half-closed by that item.** `applyReading` (`apps/web/src/components/live-svg/schematic-telemetry-context.tsx`) sets `lastSeenMs = new Date(r.time).getTime()` and `deriveStatus` computes `Date.now() - lastSeenMs > FRESH_MS`. `F4.36` closed the **NaN** route by requiring `time` to be `Date.parse`-able, but not the arithmetic: a future timestamp makes the difference **negative**, which is also not `> FRESH_MS`, so the asset reports `running` — and stays that way until a genuine reading arrives, which for a dead asset is never. Same symptom, different input, and it survives a page reload because any reconnecting client re-receives the poisoned value. **The fix belongs at the sink, not the schema**: `resolveSamples` trusts `sample.at` from the adapter, so an RTU with a skewed clock emits future timestamps *legitimately* — the unclamped `sample.at` already deferred to `F1.7`, and `F4.28` measured the PHE pilot writing **33 minutes ahead of `now()`** in production. A server-side reject would therefore delete real telemetry to fix a client-side arithmetic bug. Recommended: clamp in the web client (`Math.min(new Date(r.time).getTime(), Date.now())`), so staleness degrades toward *offline* — the safe direction for a BMS. Alternative, if a server-side bound is wanted anyway, is a generous skew window counted through the existing `onDropped`. Also worth deciding here: `Date.parse` accepts zone-less forms (`"2026-08-14 15:00:00"`), which each browser then reads in its **own** local zone — measured up to ±14 h of stale-difference per operator. Neither producer emits those today (both use `toISOString()`), so it only bites a drifted producer | P1 | 1 | 1 | — | ⬜ |
