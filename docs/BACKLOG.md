...
WAVE 1  F1.2 F1.3 F1.4 F1.5 F1.6 F1.7 F1.10  [F2.2 ✅] F2.4  F3.7 F3.10 F3.1 F3.4 F3.11
        F4.5 F4.7 F4.8 [F4.10 ✅] F4.14 F4.23 [F4.28 ✅] [F4.29 ✅] F4.30 F4.31 F4.32 F4.33 F4.34 F4.35 F4.36  [E1.7 ✅] E3.1 E5.4
...
| F4.34 | The API's telemetry `LISTEN` has no error handler and no reconnect — **raised by the `F1.1` §6 commit 4 security review (2026-08-14)**, which used it to narrow an overclaim in ADR 0016 Amendment 3. `apps/api/src/telemetry/telemetry-notify.service.ts` connects once and issues `LISTEN bms_telemetry` with no `client.on("error")` and no reconnect path. A dropped listener connection therefore reproduces **exactly** the outage §6 commit 4 was written to remove — rows landing, every dashboard dead, no error and no alarm — one hop downstream of ingest, with the ingest health body still reporting `notify=on` and `written=` climbing. Commit 4 made that state unreachable *by ingest configuration* only; this is the remaining half. Pre-existing since ADR 0002/0007 and not caused by that item. Fix is an `error` handler plus reconnect-with-backoff and a re-`LISTEN` on reconnect; the open question worth deciding with it is whether the API should surface listener health at all, since today nothing does | P1 | 1–2 | 1 | — | ⬜ |
