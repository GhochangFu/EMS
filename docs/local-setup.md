...
| drizzle journal | a staged `.sql` has no journal entry, a journal entry has no `.sql`, `when` does not strictly increase, a `when` is more than one hour ahead of the wall clock, or a `when` is not a finite number |
