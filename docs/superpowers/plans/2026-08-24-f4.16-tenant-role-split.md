...
```bash
DATABASE_URL=postgres://bms_app:bms_app_dev@localhost:5432/bms pnpm --filter api test role-grants
```

Expected: FAIL — `expected [] to deeply equal [ 'bms_auth', 'bms_fleet', 'bms_tenant' ]`.
...
```bash
DATABASE_URL=postgres://bms_app:bms_app_dev@localhost:5432/bms pnpm db:migrate
```
...
```bash
DATABASE_URL=postgres://bms_app:bms_app_dev@localhost:5432/bms BMS_TENANT_PASSWORD=bms_tenant_dev BMS_FLEET_PASSWORD=bms_fleet_dev BMS_AUTH_PASSWORD=bms_auth_dev pnpm --filter @bms/db roles
```
...
export async function assertTenantWithoutContextSeesNothing(
  ...
  const { rows } = await tenantPool.query("select 1 from bms.locations limit 1");
  expect(rows).toEqual([]);
}
...
describe.skipIf(!connectionString)("F4.16 — tenant context", () => {
  ...
  it("shows nothing when no organization is set", async () => {
    await assertTenantWithoutContextSeesNothing(tenantPool);
  });
...
```bash
DATABASE_URL=postgres://bms_app:bms_app_dev@localhost:5432/bms pnpm --filter api test tenant-context
```
...
-- FORCE, not just ENABLE: without FORCE the table owner silently bypasses the
-- policy, which is the defect decision 8 exists to close.
...
```bash
DATABASE_URL=postgres://bms_app:bms_app_dev@localhost:5432/bms pnpm db:migrate && DATABASE_URL=postgres://bms_app:bms_app_dev@localhost:5432/bms pnpm --filter api test tenant-context
```

Expected: PASS, four tests.

- [ ] **Step 7: Re-run the seed, because FORCE binds the owner too**

```bash
DATABASE_URL=postgres://bms_app:bms_app_dev@localhost:5432/bms pnpm db:seed
```

Expected: PASS. `bms_app` is the owner and `FORCE ROW LEVEL SECURITY` now
constrains it, so a seed insert into these five tables must satisfy the policy.
If it fails, the seed needs a `set_config('app.current_organization', ...)` per
organization block — fix it in this task, not later; a broken seed breaks every
integration suite.
...
```bash
pnpm --filter api test database-urls
```
...
```bash
pnpm --filter api test database-urls && pnpm typecheck
```
...
```bash
pnpm --filter api test identity-bootstrap
```
...
```bash
DATABASE_URL=postgres://bms_app:bms_app_dev@localhost:5432/bms pnpm --filter api test && pnpm typecheck
```
