...
/**
 * `field: z.array(...)…` and the `.max(NAME)` on it, read from source.
 *
 * **Whitespace is collapsed first**, so the scan cannot be defeated by the
 * formatter. `assetPoints: z.array(onboardingDraftAssetPointSchema).max(MAX_ONBOARDING_ASSET_POINTS).optional(),`
 * is exactly at the print width, and a wrap into `z\n.array(…)\n.max(…)` would
 * otherwise hide one declaration from a single-line pattern — the guard below
 * would fire and the failure would look like a missing cap rather than a
 * reformatted line.
 *
 ...
 *
 * - a cap applied somewhere other than the array declaration line — a
 *   `.superRefine` on the object, or a `.max()` on a schema constant declared
...
function declaredCaps(rel: string): Map<string, string | null> {
  ...
  for (const match of source.matchAll(
    /\b(rtus|pointKeys|assets|assetPoints)\s*:\s*z\.array\([^)]*\)((?:\s*\.\w+\([^)]*\))*)/g,
  )) {
    const field = match[1] as string;
    const chain = match[2] ?? "";
    const cap = /\.max\(\s*([A-Za-z0-9_]+)\s*\)/.exec(chain);
  ...
  // declarations are reshaped, fix this parser; do not delete the assertion.
  if (found.size < FIELDS.length) {
    throw new Error(
      `only found ${found.size} of ${FIELDS.length} draft array declarations in ${rel} ` +
        `(${[...found.keys()].join(", ") || "none"}). The four arrays are two declarations of ` +
