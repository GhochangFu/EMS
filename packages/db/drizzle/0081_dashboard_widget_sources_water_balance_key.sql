-- E4.3 / ADR 0073 decision 3 — one more parameterised catalog entry.
--
-- This migration widens `dashboard_widget_sources_catalog_key_check` to accept
-- `'water.balance'`, and does nothing else. It lands with the `metricCatalogKeySchema` member it
-- admits and the resolver that answers it
-- (`apps/api/src/dashboard-builder/metric-catalog.service.ts`).
--
-- `0079` IS NOW FROZEN, in its own turn. It widened `0054`'s five to the seven this file's own
-- CHECK still lists before the ADD below runs; from here the EFFECTIVE list is this file's
-- eight, exactly the `0054` / `0079` pattern one generation on.
--
...
-- exists to refuse. `0079` is frozen (the pre-commit hook), so the widening is a new file, in
-- the `0079` / `0055` / `0050` pattern that admitted the entries before this one.
--
-- WHY `DROP IF EXISTS` + `ADD` AND NOT AN `IF NOT EXISTS` GUARD. The constraint ALREADY
-- EXISTS, carrying `0079`'s seven-value list. An existence check would find it, conclude there
-- was nothing to do, and skip the widening while reporting a successful migration — a
-- database that refuses every water-balance binding with a green migrate. DROP IF EXISTS
-- followed by ADD is idempotent on its own: a second run drops the widened constraint and
-- re-adds it identically. `0079`'s header records the same reasoning.
--
...
-- accepts, so no stored row can fail it and an older `apps/api` simply never writes the new
-- key. Forward-only (§4.4); no down migration, per this repository's convention.
--
...
-- bracket — `0051`, `0053`, `0055` and `0079` record the same rule.
--
...
-- build rather than a page; the same file pins `0054`'s frozen five and `0079`'s frozen seven so
-- the three lists cannot be confused.
...
ALTER TABLE bms.dashboard_widget_sources
  ...
  CHECK (catalog_key IN ('alarms.active.count', 'alarms.active', 'workorders.open.count', 'workorders.open', 'assets.health.score', 'sustainability.total', 'sustainability.by_location', 'water.balance'));
