...
export async function verifyHierarchySeed(
  ...
  // Every table below is outside the `0040` policy set, so these counts are
  // unaffected by the tenant split and stay in one query.
  ...
  expect("PHE assets", g.phe_assets, 48);
  ...
  expect("PHE asset_points", g.phe_points, 252);
  expect("assets without location_id", g.orphan_assets, 0);
  expect("asset/RTU location mismatch", g.loc_mismatch, 0);
  ...
  await withOrganization(pool, phewbOrgId, async () => {
    const res = await pool.query<{ phe_locs: string; phe_rtus: string }>(`
      SELECT
        (SELECT COUNT(*)::text FROM bms.locations l
          INNER JOIN bms.organizations o ON o.id = l.organization_id
