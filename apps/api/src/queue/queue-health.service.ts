 ...
 * metrics registry and a Redis `GET` of the heartbeat key.
 *
 * The tick is read through the heartbeat queue's own connection — the one
 * BullMQ already holds for that queue — so the API opens no second Redis
 * client for the probe. `handle.client` awaits the connection, which is why
