...
export async function parameterPresentWrites(): Promise<void> {
  ...
  const { deps, writes, skips, status } = buildSweepDeps([cost], samples, {
    parameters,
  });
  ...
  assert(
    batch.length === 1 &&
      batch[0]?.pointKey === "COST" &&
      batch[0].value === 21.5,
    `kw 10 × f 2.15 must write 21.5, got ${JSON.stringify(batch)}`,
  );
  assert(
    skips.length === 0,
    `no skip when the parameter is present, got ${JSON.stringify(skips)}`,
  );
  assert(
    status.get("asset-1", "tp-1")?.outcome === "written",
    "the registry records the write",
  );
}

/** (ii) the owed guard: the parameter absent → parameter_unset, no row */
export async function parameterAbsentRefusesWithoutARow(): Promise<void> {
  const cost = def({
    ...V3,
    pointKey: "COST",
    templatePointId: "tp-cost",
    formula: "{kw} * $f",
  });
  const plain = def({
    pointKey: "DOUBLE",
    templatePointId: "tp-plain",
    formula: "{kw} * 2",
  });
  const samples = new Map([["asset-1:kw", { value: 10, timeMs: 0 }]]);
  const { deps, writes, skips, status, warnings } = buildSweepDeps(
    [cost, plain],
    samples,
  );
  await runScheduledSweep(deps, new Map(), 0);
  ...
  assert(
    batch.length === 1 &&
      batch[0]?.pointKey === "DOUBLE" &&
      batch[0].value === 20,
    `only the v1 formula writes; the v3 one has no parameter. Got ${JSON.stringify(batch)}`,
  );
  assert(
    batch.every((w) => w.pointKey !== "COST"),
    "no row for COST — an absent parameter is never 0",
  );
  assert(
    count(skips, "parameter_unset") === 1 && skips.length === 1,
    `exactly one parameter_unset, got ${JSON.stringify(skips)}`,
  );
  ...
  assert(
    status.get("asset-1", "tp-plain")?.outcome === "written",
    "the v1 formula's registry entry is written",
  );
  assert(
    warnings.length === 0,
    `an unset parameter is a counted refusal, not a warning, got ${JSON.stringify(warnings)}`,
  );
}

/** (iii) the parameter read throws → parameters_unresolved, v2 and v1 unaffected */
export async function failedReadRefusesOnlyParameterHolders(): Promise<void> {
  const cost = def({
    ...V3,
    pointKey: "COST",
    templatePointId: "tp-cost",
  ...
  const samples = new Map([["asset-1:kw", { value: 10, timeMs: 0 }]]);
  const { deps, writes, skips, warnings } = buildSweepDeps(
    [cost, v2, plain],
    samples,
    { parametersThrows: true },
  );
  await runScheduledSweep(deps, new Map(), 0);
  const written = writes
    .flat()
    .map((w) => `${w.pointKey}=${w.value}`)
    .sort()
    .join(",");
  assert(
    written === "DOUBLE=20,HALF=5",
    `the v2 and v1 formulas still write when the parameter read fails, got ${written}`,
  );
  assert(
...
export async function pairsAreTheDistinctSetOncePerSweep(): Promise<void> {
  const a1 = def({
    ...V3,
    assetId: "asset-1",
    pointKey: "COST",
  ...
  const { deps, writes, parameterRequests } = buildSweepDeps(
    [a1, a2, again, v2],
    samples,
    { parameters },
  );
  await runScheduledSweep(deps, new Map(), 0);
  assert(
    parameterRequests.length === 1,
    `one parameter read per sweep, got ${parameterRequests.length}`,
  );
  const pairs = parameterRequests[0]
    .map((p) => `${p.assetId}:${p.key}`)
    .sort()
    .join(",");
  assert(
  ...
  const written = writes
    .flat()
    .map((w) => `${w.assetId}:${w.pointKey}=${w.value}`)
    .sort()
    .join(",");
  assert(
...
export async function aDollarFreeV3MakesNoRead(): Promise<void> {
  ...
  await runScheduledSweep(deps, new Map(), 0);
  assert(
    parameterRequests.length === 0,
    "no v3 definition holds a $key, so no read is made",
  );
  assert(
    writes.flat().length === 1,
    "and the formula writes even though the read would have thrown",
  );
}
