...
export async function cleanup(pool: pg.Pool): Promise<void> {
  ...
  await pool.query(`DELETE FROM bms.asset_templates WHERE code = $1`, [TEST_TEMPLATE_CODE]);
}
...
export async function publishFixtureTemplate(
  ...
  return svc.publish(fx.adminJwt, draft.id);
}
...
export async function assertOnlyPublishedTemplatesInstantiate(
  ...
  await svc.deleteDraft(fx.adminJwt, draft.id);
}
