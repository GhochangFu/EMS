...
/**
 * The electrical pack of the stock asset-template catalog — `F2.13` ships the
 * feeder/incomer class; `F2.12` adds transformer, DG set, UPS, solar PV and
 * APFC to this same file (plan §11). ADR 0052 decisions 1, 2 and 6.
 *
 * ---
 *
...
/** A measured point's five null calc fields, spelled once. Spread first. */
const MEASURED = {
  kind: "measured",
...
const CORE = { tier: "core" } as const;
/** Tier `X` — `required: false` beside it, always. */
const EXTENDED = { tier: "extended" } as const;

const ELECTRICAL_FEEDER: StockAssetTemplateEntry = {
...
/**
 * The pack. One entry today; `F2.12` appends five. Each entry carries its own
 * `stockVersion` — never a shared constant (ADR 0052 decision 2).
 */
