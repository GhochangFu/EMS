 ...
 *    tag list changes, these change with it, in the same PR. **That block
 *    lives in `electrical-classes-3.spec.ts` since `E4.1c`** (plan design
 *    decision 17): this file stood at 999 lines against the §4.5 cap, and the
 *    six `bms-calc-v3` rows the feeder gains there need their own claims.
 *    Only `FEEDER_CODE` stays here, exported. **The feeder's deferral half is
 *    not here either**: its "no derived point, no `content.kpis`" guard moved
 *    to `stock-catalog-deferrals.spec.ts` with `DEFERRAL_REASON`, the text it
 *    fails with.
 *
...
export const FEEDER_CODE = "electrical-feeder";
