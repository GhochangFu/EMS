 ...
 *    `minCoverageRatio` governs a `@scope` aggregate only (ADR 0055 decision
 *    11); a window `sum` refuses `window_sparse` below 90% coverage of the
 *    elapsed window (ADR 0070 Amendment 3, `E4.4`) and a window with no
 *    samples refuses `window_empty` — these rows are `avg` and `delta`, so
 *    neither guard touches them; `start_count` is tier X, so an asset without it
...
export const MECHANICAL_ESCALATOR: StockAssetTemplateEntry = {
  ...
  points: [
    ...
    // decision 11); a window `sum` refuses `window_sparse` below 90%
    // coverage of the elapsed window (ADR 0070 Amendment 3, `E4.4`) and a
    // window with no samples refuses `window_empty` — these rows are `avg`
    // and `delta`, so neither guard touches them; no `meta`; a count carries
