 ...
 *    11); this row's `sum` refuses `window_sparse` below 90% coverage of the
 *    elapsed window (ADR 0070 Amendment 3, `E4.4`) and a window with no
 *    samples refuses `window_empty`; (4) `fire_isolate_state` is tier
...
export const FACILITY_FIRE_PANEL: StockAssetTemplateEntry = {
  ...
  points: [
    ...
    // samples refuses `window_empty`, no `meta`. `this_month` is
