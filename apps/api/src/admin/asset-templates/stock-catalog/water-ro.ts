 ...
 *    client-confirmed.
 *  - `water-ro` **v2** (2026-09-19, `E4.1c`): three `bms-calc-v3` derived
 ...
 *    over its own inlet flow): `kl_today = sum({feed_flow_klh}, today)`,
 *    `water_cost_today = sum({feed_flow_klh}, today) * $water_tariff_per_kl`,
 *    `water_saving_vs_baseline_pct` against `$water_baseline_kl_per_day`
 *    prorated by `hours(today)`. **The inlet here is `feed_flow_klh`** —
 *    the feed flow (permeate plus reject) — the water the skid draws. Four
 ...
 *  - `water-ro` **v4** (2026-09-22, `E4.2` PR 2 post-merge sweep): no new
 ...
 *  - `water-ro` **v5** (2026-09-23, `E4.3` PR 2, U6): three more
 *    `bms-calc-v3` derived points appended at `sortOrder` 25–27 (ADR 0073
 *    decision 4) — `outlet_kl_today = sum({permeate_flow_klh}, today)` and
 *    its `_this_month` / `_this_year` calendar siblings, the same
 *    scheduled/window/coverage rules as the inlet rows above. `reject_flow_klh`
 *    is not an outlet here: this skid is `internal` on any site where its
 *    reject feeds the ETP, and the ETP's discharge already counts it.
 *
...
export const WATER_RO: StockAssetTemplateEntry = {
  ...
  description:
    ...
    "the permeate leaving the skid, the same estimate shape as the four above.",
  stockVersion: 5,
  content: {
    contentVersion: 1,
    alarms: [
  ...
  points: [
    ...
    {
      ...derived("sum({feed_flow_klh}, today)", { calcTrigger: "scheduled", calcIntervalSeconds: 60, formulaDialect: CALC_DIALECT_V3 }),
      pointKey: "kl_today",
    ...
    {
      ...derived("sum({feed_flow_klh}, today) * $water_tariff_per_kl", { calcTrigger: "scheduled", calcIntervalSeconds: 60, formulaDialect: CALC_DIALECT_V3 }),
    ...
    {
      ...derived("(1 - sum({feed_flow_klh}, today) / ($water_baseline_kl_per_day * hours(today) / 24)) * 100", { calcTrigger: "scheduled", calcIntervalSeconds: 60, formulaDialect: CALC_DIALECT_V3 }),
    ...
    // this class's own outlet (permeate_flow_klh).
    {
      ...derived("sum({permeate_flow_klh}, today)", { calcTrigger: "scheduled", calcIntervalSeconds: 60, formulaDialect: CALC_DIALECT_V3 }),
    ...
    {
      ...derived("sum({permeate_flow_klh}, this_month)", { calcTrigger: "scheduled", calcIntervalSeconds: 60, formulaDialect: CALC_DIALECT_V3 }),
    ...
    {
      ...derived("sum({permeate_flow_klh}, this_year)", { calcTrigger: "scheduled", calcIntervalSeconds: 60, formulaDialect: CALC_DIALECT_V3 }),
